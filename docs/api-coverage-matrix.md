...
| QuillData/SwiftData shape | partial | partial-real | yes | yes | SQLite JSON-row store supports insert/fetch/delete/sort/value-predicate/upsert/relationships, `hasChanges`, stable `id`/`name`/`slug` identity, and class identity-map saves; nonthrowing insert failures surface on `save()`. | Macro/source-compatible `@Model` and class-predicate lowering strategy, schema/indexing, migrations, and relationship delete rules. |
