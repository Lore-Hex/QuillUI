...
| `Network` | `IPv4Address`, `IPv6Address`, `NWPathMonitor` initial `currentPath`, `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWInterface` values returned by scoped address/host parsing, `NWEndpoint.Port` parsing/known constants/properties/debug text/equality/hash behavior, scoped and unscoped `NWEndpoint.Host.init(_:)` classification/description/equality/hash behavior, and common `NWEndpoint` host-port/service/Unix path value descriptions, equality, and hashing now have Apple-checked `Parity` rows for current tested contracts. `NetworkPathInterfaceParityTests` pins the path monitor pre-start state, path/interface enum string, resolved scoped interface value, equality, and hash contracts. `NetworkIPAddressParityTests` captures the IPv4/IPv6 parser, legacy IPv4 single-component wrapping, dotted octal/hex IPv4 edge cases, data length, classifier-boundary, multicast-scope, IPv4 mapping, empty/unresolved IPv6 scope fallback, string, debug-output, address equality/hash, and `IPv6Address.Scope` raw-value edge matrix observed on Apple Network. `NetworkEndpointPortParityTests` now covers port parser, constant, equality, and hash semantics. `NetworkEndpointHostParityTests` now covers scoped parsed-to-direct host equality/hash, empty/unresolved/malformed scope fallback classification, and scoped host-port endpoint equality/hash. | Live path monitoring, synthetic constructed interfaces, connections, listeners, DNS, TLS, UDP/TCP, real path probing, and NetworkExtension VPN behavior are incomplete or fallback-only. |
