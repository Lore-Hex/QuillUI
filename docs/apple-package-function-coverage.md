...
| Native secure persistence, access-control enforcement, keychain sharing, synchronization, cross-process lookup, native key validation/handles, native cryptographic key generation/sign/verify, key agreement, Secure Enclave policy | Incomplete | Required before claiming KeychainSwift or Security keychain parity. |
