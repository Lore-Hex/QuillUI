...
| `SecItem` key-class rows (`kSecClassKey`, `kSecAttrApplicationTag`, `kSecAttrApplicationLabel`, `kSecAttrKeyClass`, `kSecAttrKeyType`, key size, permanence, and capability metadata) | Usable source compatibility | Stores, matches, and returns process-local key metadata/data and synthesized `SecKey` references for Signal/libsignal-style imported key material. It does not validate native key encodings, bind to native key handles, generate keys, sign/verify, perform key agreement, enforce access-control/Secure Enclave policy, or persist outside the process. |
...
| `KeychainSwift` and lower-level `Security` random/keychain storage | Upstream-shaped in-memory `KeychainSwift` UTF-8 string storage, raw data storage, single-byte bool storage, `getData(_:asReference:)`, deterministic reference handles, `allKeys`, delete, access-group/synchronizable namespaces, accessibility option metadata, result-code tracking, prefix read isolation, and namespace `clear()` behavior; `SecRandomCopyBytes` for Signal-shaped key-material generation; process-local `SecKeyCreateWithData`, `SecKeyCopyAttributes`, `SecKeyCopyExternalRepresentation`, and synthesized `SecKey` references for imported key material; process-local `SecItemAdd`, `SecItemCopyMatching`, `SecItemUpdate`, and `SecItemDelete` generic-password, internet-password, and key-class rows with access-control metadata, authentication/use query controls, server/protocol/authentication/port/path endpoint separation, key-item application-tag/application-label/key-class/key-type/key-size/capability metadata, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and persistent-reference handles for account storage. | Secure OS keychain persistence, native access-control enforcement, true keychain sharing, iCloud synchronization, native item protection, cross-process behavior, native key validation/handles, key generation/sign/verify, key agreement, Secure Enclave behavior, and cryptographic equivalence beyond the valid-count random-byte contract. |
...
| Native secure persistence, access-control enforcement, keychain sharing, synchronization, cross-process lookup, native key validation/handles, key generation/sign/verify, key agreement, Secure Enclave policy | Incomplete | Required before claiming KeychainSwift or Security keychain parity. |
