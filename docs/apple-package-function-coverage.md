...
| `PhotosUI` / `Photos` | Local Linux `Photos` fallback saves `UIImage` assets, fetches `PHAsset` rows, loads image data/resized thumbnails through `PHImageManager`, and file-backed `PhotosPickerItem` values load Transferable file imports through `NSItemProvider`. | Authorization is deterministic, `PhotosUI` picker UI/native item providers and real system photo-library integration are incomplete. |
...
| Picker UI, native picker item providers, real photo-library service integration, limited-library permissions, video thumbnails | Incomplete | Required for Photos parity. |
