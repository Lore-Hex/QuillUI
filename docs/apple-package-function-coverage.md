...
| `CoreGraphics` | Swift-corelibs geometry re-export, affine/path value behavior, bitmap `CGContext` fills/strokes/images/gradients/masks, blend modes, shadows, transparency layers, current-path introspection, font rendering state flags, and Cairo-backed `CGContext` forwarding for paths, fill rules, blend/shadow state, miter limit, and antialias toggles are `Usable` for current geometry, SceneKit interop, image rendering, and custom-view drawing. | Event sources, key state, keyboard events, event posting, pointer events, event taps, and broad drawing APIs beyond the focused bitmap/Cairo drawing surface are incomplete. |
...
| `CGContext` bitmap/state drawing (`fill`, `stroke`, `draw`, gradients, masks, blend modes, shadows, transparency layers, miter limit, antialias/font toggles) | Usable | Direct bitmap contexts now cover current app/image-rendering needs, including separable/non-separable blend modes, alpha masks, blurred shadows, transparency-layer compositing, and saved/restored font smoothing/subpixel state. GTK and Qt Cairo backends receive fill/stroke/path/image drawing state, including blend mode, shadow, miter limit, and effective antialiasing. Cairo shadow blur remains approximate/no-blur in the toolkit backend. |
