...
| `CoreGraphics` | Swift-corelibs geometry re-export, `CGAffineTransform` math, `CGPoint.applying(_:)`, `CGPathElement`/`CGPathElementType`, `CGPath.applyWithBlock`, path copy/transform recording, rect/rounded-rect/ellipse/curve/arc recording, `CGPath.isEmpty`, `currentPoint`, `boundingBox`, `boundingBoxOfPath`, winding/even-odd `contains(_:using:transform:)`, and Cairo-backed `CGContext` rect-list/path/quad/cubic/fill-rule forwarding are `Usable` for current geometry/path interop and custom-view drawing. | Event sources, key state, keyboard events, event posting, pointer events, event taps, and drawing APIs beyond focused geometry/path/context forwarding are incomplete. |
...
| `CGContext.addRects`, `addPath`, `addQuadCurve`, `addCurve`, `fillPath(using:)`, `clip(using:)` | Usable | The Linux drawing shim forwards rect lists, path element streams, and direct quad/cubic curves into the pluggable Cairo backend, including even-odd/winding fill-rule selection for fill and clip. This is scoped backend forwarding, not full CoreGraphics raster parity. |
