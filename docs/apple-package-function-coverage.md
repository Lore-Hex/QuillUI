...
| `PhotosUI` / `Photos` | Local Linux `Photos` fallback saves `UIImage` assets, fetches `PHAsset` rows, and loads image data/resized thumbnails through `PHImageManager`. | Authorization is deterministic, `PhotosUI` picker UI and transferable item loading are compile-only, and real system photo-library integration is incomplete. |
...
| Picker UI, transferable item loading, real photo-library service integration, limited-library permissions, video thumbnails | Incomplete | Required for Photos parity. |
