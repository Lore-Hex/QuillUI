...
| `Security` | `SecRandomCopyBytes`, process-local `SecItemAdd`, `SecItemCopyMatching`, `SecItemUpdate`, `SecItemDelete`, persistent-reference returns/lookups/deletes, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and current keychain constants. | Native secure keychain persistence/access control, OS-enforced keychain sharing, real keychain synchronization, cross-process lookup, certificate parsing, policy evaluation, platform trust store, and Secure Transport parity are incomplete. |
...
| `SecItemAdd(_:_:)` | Usable | Process-local item creation for generic-password and keychain-shaped rows. Detects duplicate identity across class, service, account, access group, and synchronizable attributes, and can return attributes, data, object references, or persistent-reference `Data`. Not persistent or secure. |
| `SecItemCopyMatching(_:_:)` | Usable | Process-local matching with data, attributes, object reference, persistent reference, mixed data/persistent-reference dictionaries, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny` local-plus-synchronizable matching, not-found status, and `kSecMatchLimitAll` array results for current Signal/keychain storage contracts. |
...
| `SecItemDelete(_:)` | Usable | Deletes matching process-local items, including delete-by-persistent-reference, and returns not-found when no row matches. |
...
| `kSecRandomDefault`, `kSecClass*`, `kSecAttr*`, `kSecAttrSynchronizableAny`, `kSecValue*`, `kSecReturn*`, `kSecMatchLimit*`, `errSec*` constants | Usable | Source-compatible constants for current Security, KeychainSwift-adjacent, and Signal-shaped storage contracts. |
...
| Lower-level `Security` `SecItem` generic-password add/copy/update/delete | Usable | Supports the process-local keychain-shaped rows, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and persistent-reference handles Signal-style libraries commonly source-target before a secure backend is attached. |
