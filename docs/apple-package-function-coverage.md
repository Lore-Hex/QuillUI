...
| `Network` | `IPv4Address`, `IPv6Address`, `NWPath.Status`, `NWInterface.InterfaceType`, `NWEndpoint.Host`, `NWEndpoint.Port` parsing/known constants/debug text, and common `NWEndpoint` value descriptions now have Apple-checked `Parity` rows for current tested contracts. | Path monitoring, constructed interfaces, connections, listeners, DNS, TLS, UDP/TCP, and real path probing are incomplete or fallback-only. |
