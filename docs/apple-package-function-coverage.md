...
| `Network` | `IPv4Address`, `IPv6Address`, `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWEndpoint.Port` parsing/known constants/properties/debug text/equality/hash behavior, scoped and unscoped `NWEndpoint.Host.init(_:)` classification/description/equality/hash behavior, and common `NWEndpoint` host-port/service/Unix path value descriptions, equality, and hashing now have Apple-checked `Parity` rows for current tested contracts. `NetworkPathInterfaceParityTests` pins the path/interface enum string, equality, and hash contracts. `NetworkIPAddressParityTests` captures the IPv4/IPv6 parser, data length, classifier-boundary, multicast-scope, IPv4 mapping, string, debug-output, address equality/hash, and `IPv6Address.Scope` raw-value edge matrix observed on Apple Network. `NetworkEndpointPortParityTests` now covers port parser, constant, equality, and hash semantics. `NetworkEndpointHostParityTests` now covers scoped parsed-to-direct host equality/hash and scoped host-port endpoint equality/hash. | Path monitoring, constructed interfaces, connections, listeners, DNS, TLS, UDP/TCP, real path probing, and NetworkExtension VPN behavior are incomplete or fallback-only. |
...
| `IPv4Address.init?(String)` / `init?(Data)` / `String(describing:)` / `debugDescription` | Parity | Apple-checked IPv4 parser covers strict whitespace rejection, legacy 1-4 component forms, octal and hex components, 32-bit wraparound, 4-byte data validation, scoped interface literals through resolved OS interface names, canonical dotted-decimal string/debug output, and equality/hash coherence across parsed, data-backed, and scoped name-backed values. `NetworkIPAddressParityTests` adds Apple-observed 3/4/5-byte data initializer boundaries, classifier-edge string/debug checks, and direct address equality/hash checks. Linux also exposes `.description` as a compatibility alias, but direct `.description` is not an Apple public API for this value. |
...
| `IPv6Address.init?(String)` / `init?(Data)` / `String(describing:)` / `debugDescription` | Parity | Apple-checked IPv6 parser covers `inet_pton` literal acceptance, whitespace rejection, 16-byte data validation, scoped interface literals through resolved OS interface names, canonical `inet_ntop` string/debug output, and equality/hash coherence across parsed, data-backed, and scoped name-vs-numeric-interface values. `NetworkIPAddressParityTests` adds Apple-observed 15/16/17-byte data initializer boundaries, string/debug checks for classifier edges, and direct address equality/hash checks. Linux also exposes `.description` as a compatibility alias, but direct `.description` is not an Apple public API for this value. |
