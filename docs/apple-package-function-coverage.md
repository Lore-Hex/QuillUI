...
| `SecItem` key-class rows (`kSecClassKey`, `kSecAttrApplicationTag`, `kSecAttrApplicationLabel`, `kSecAttrKeyClass`, `kSecAttrKeyType`, key size, permanence, and capability metadata) | Usable source compatibility | Stores, matches, and returns process-local key metadata/data for Signal/libsignal-style imported key material. It does not generate keys, sign/verify, perform key agreement, enforce access-control/Secure Enclave policy, or persist outside the process. |
...
| `kSecRandomDefault`, `kSecClass*`, `kSecAttr*`, `kSecAttrSynchronizableAny`, `kSecValue*`, `kSecReturn*`, `kSecMatchLimit*`, `kSecUse*`, common internet-password protocol/authentication constants, key-item constants, `errSec*` constants | Usable | Source-compatible constants for current Security, KeychainSwift-adjacent, and Signal-shaped storage contracts, including server/security-domain/protocol/authentication/port/path endpoint attributes, key application-tag/application-label/key-class/key-type/key-size/capability attributes, and authentication/use query controls. |
...
| Native secure persistence, access-control enforcement, keychain sharing, synchronization, cross-process lookup, key generation/sign/verify, key agreement, Secure Enclave policy | Incomplete | Required before claiming KeychainSwift or Security keychain parity. |
