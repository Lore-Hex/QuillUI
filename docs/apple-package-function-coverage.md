...
| `CloudKit` | None yet beyond compile/fallback shapes. | Record/query/database/account/subscription behavior is incomplete. `cocologics/OpenCloudKit` is documented as a possible provider boundary, not vendored runtime behavior. |
...
| `UNUserNotificationCenter.requestAuthorization(...)`, `notificationSettings()` | Partial | Routes through `QuillNotificationService`; authorization status is configurable and process-local. |
