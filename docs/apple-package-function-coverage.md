...
| `Network` | `IPv4Address`, `IPv6Address`, `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWEndpoint.Port` parsing/known constants/properties/debug text, scoped and unscoped `NWEndpoint.Host.init(_:)` classification/description/equality/hash behavior, and common `NWEndpoint` host-port/service/Unix path value descriptions, equality, and hashing now have Apple-checked `Parity` rows for current tested contracts. `NetworkPathInterfaceParityTests` pins the path/interface enum string contracts. `NetworkIPAddressParityTests` captures the IPv4/IPv6 parser, data length, classifier-boundary, multicast-scope, IPv4 mapping, string, and debug-output edge matrix observed on Apple Network. `NetworkEndpointHostParityTests` now covers scoped parsed-to-direct host equality/hash and scoped host-port endpoint equality/hash. | Path monitoring, constructed interfaces, connections, listeners, DNS, TLS, UDP/TCP, real path probing, and NetworkExtension VPN behavior are incomplete or fallback-only. |
