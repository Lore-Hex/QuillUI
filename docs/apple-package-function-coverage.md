...
| `Security` | `SecRandomCopyBytes`, process-local `SecItemAdd`, `SecItemCopyMatching`, `SecItemUpdate`, `SecItemDelete` generic-password and internet-password rows, persistent-reference returns/lookups/deletes, access-control metadata, authentication/use query controls, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, server/security-domain/protocol/authentication/port/path endpoint identity, and current keychain constants. | Native secure keychain persistence/access-control enforcement, OS-enforced keychain sharing, real keychain synchronization, cross-process lookup, certificate parsing, policy evaluation, platform trust store, and Secure Transport parity are incomplete. |
...
| `SecItemAdd(_:_:)` | Usable | Process-local item creation for generic-password and internet-password rows. Detects duplicate identity across class, service, account, server, security domain, protocol, authentication type, port, path, access group, and synchronizable attributes, and can return attributes, data, object references, or persistent-reference `Data`. Not persistent or secure. |
| `SecItemCopyMatching(_:_:)` | Usable | Process-local matching with data, attributes, object reference, persistent reference, mixed data/persistent-reference dictionaries, internet endpoint filters, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny` local-plus-synchronizable matching, not-found status, and `kSecMatchLimitAll` array results for current Signal/keychain storage contracts. |
| `SecItemUpdate(_:_:)` | Usable | Updates matching process-local item values and attributes, preserving Apple-shaped duplicate and not-found status paths for generic-password and internet-password endpoint identities. |
| `SecItemDelete(_:)` | Usable | Deletes matching process-local generic-password and internet-password items, including delete-by-persistent-reference and endpoint-scoped deletes, and returns not-found when no row matches. |
...
| `kSecRandomDefault`, `kSecClass*`, `kSecAttr*`, `kSecAttrSynchronizableAny`, `kSecValue*`, `kSecReturn*`, `kSecMatchLimit*`, `kSecUse*`, common internet-password protocol/authentication constants, `errSec*` constants | Usable | Source-compatible constants for current Security, KeychainSwift-adjacent, and Signal-shaped storage contracts, including server/security-domain/protocol/authentication/port/path endpoint attributes and authentication/use query controls. |
...
| Lower-level `Security` `SecItem` generic-password and internet-password add/copy/update/delete | Usable | Supports process-local keychain-shaped rows, access-control metadata, authentication/use query controls, server/protocol/authentication/port/path endpoint separation, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and persistent-reference handles Signal-style libraries commonly source-target before a secure backend is attached. |
