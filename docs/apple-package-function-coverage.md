...
| `Security` | `SecRandomCopyBytes`, process-local `SecItemAdd`, `SecItemCopyMatching`, `SecItemUpdate`, `SecItemDelete`, and current keychain constants. | Native secure keychain persistence/access control, access groups, synchronization, cross-process lookup, certificate parsing, policy evaluation, platform trust store, and Secure Transport parity are incomplete. |
