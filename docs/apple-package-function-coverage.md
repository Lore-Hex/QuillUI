...
| `Network` | `IPv4Address`, `IPv6Address`, `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWEndpoint.Port` parsing/known constants/properties/debug text, unscoped `NWEndpoint.Host.init(_:)` classification/description behavior, and selected common `NWEndpoint` value descriptions now have Apple-checked `Parity` rows for current tested contracts. | Path monitoring, constructed interfaces, connections, listeners, DNS, TLS, UDP/TCP, real path probing, and scoped IPv6 endpoint interface metadata are incomplete or fallback-only. |
...
| `IPv4Address.init?(String)` / `init?(Data)` / `String(describing:)` / `debugDescription` | Parity | Apple-checked IPv4 parser covers strict whitespace rejection, legacy 1-4 component forms, octal and hex components, 32-bit wraparound, 4-byte data validation, and canonical dotted-decimal string/debug output. Linux also exposes `.description` as a compatibility alias, but direct `.description` is not an Apple public API for this value. |
...
| `IPv4Address.interface` / `isLoopback` / `isLinkLocal` / `isMulticast` | Parity | Apple-checked nil interface for literal/data construction plus exact loopback, `169.254/16` link-local, and `224/4` multicast classification for tested edge cases. |
| `IPv6Address.init?(String)` / `init?(Data)` / `String(describing:)` / `debugDescription` | Parity | Apple-checked IPv6 parser covers `inet_pton` literal acceptance, whitespace rejection, 16-byte data validation, and canonical `inet_ntop` string/debug output. Linux also exposes `.description` as a compatibility alias, but direct `.description` is not an Apple public API for this value. |
...
| `IPv6Address.Scope` / `interface` / `isAny` / `isLoopback` / `isIPv4Compatabile` / `isIPv4Mapped` / `asIPv4` / `is6to4` / `isLinkLocal` / `isMulticast` / `multicastScope` / `isUniqueLocal` | Parity | Apple-checked nil interface for literal/data construction, the Apple spelling `isIPv4Compatabile`, IPv4-compatible/mapped extraction behavior, `2002::/16` 6to4 detection, `fe80::/10` link-local detection, `fc00::/7` unique-local detection, and multicast scope raw values for the tested edge cases. |
| `NWEndpoint.Host.init(_:)` / direct host cases / `description` / `debugDescription` | Parity | Shared macOS/Linux `NetworkEndpointHostParityTests` cover Apple-matching unscoped IPv4 literals, IPv6 literals, DNS names, empty-string normalization to `.`, IPv4-mapped IPv6 host literals such as `::ffff:192.0.2.1`, direct `.name`, `.ipv4`, and `.ipv6` case descriptions, and debug text. |
| `NWEndpoint.Host` scoped IPv6 interface literals | Incomplete | Apple preserves interface scope in host literals such as `fe80::1%lo0`; Linux does not yet model endpoint-host scope metadata for parity. |
| `NWEndpoint.Port.init?(String)` / `init?(rawValue:)` / integer literal / `rawValue` / `String(describing:)` / `debugDescription` | Parity | Apple-checked port parsing covers leading C whitespace, plus signs, `-0`, decimal-only input, UInt16 bounds, trailing-character rejection, failable raw-value construction, integer literals, and decimal debug/string-describing output. The shared macOS/Linux `NetworkEndpointPortParityTests` contract runs against Apple `Network` on macOS and the shim on Linux. Direct `.description` is not an Apple public API for this value and is intentionally not cloned. |
...
| `NWEndpoint.hostPort`, `service`, and `unix` value `description` / `debugDescription` | Parity | Shared macOS/Linux `NetworkEndpointHostParityTests` cover Apple-checked host-port, Unix path, and valid DNS-SD service string/debug descriptions, including IPv6 host-port separator behavior, IPv4-mapped host-port normalization, and service type/domain trailing-dot normalization. |
