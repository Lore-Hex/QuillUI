...
| `CoreGraphics` | Swift-corelibs geometry re-export, `CGAffineTransform` math, `CGPoint.applying(_:)`, `CGPathElement`/`CGPathElementType`, `CGPath.applyWithBlock`, path copy/transform recording, rect/rounded-rect/ellipse/curve recording, `CGPath.isEmpty`, `currentPoint`, `boundingBox`, `boundingBoxOfPath`, and winding/even-odd `contains(_:using:transform:)` are `Usable` for current geometry/path interop. | Event sources, key state, keyboard events, event posting, pointer events, event taps, and drawing APIs beyond shared geometry/path value behavior are incomplete. |
...
| `NWError.posix(_:)` / `.dns(_:)` / `.tls(_:)` / equality / `debugDescription` / `String(describing:)` / `String(reflecting:)` / `localizedDescription` | Parity | Apple-probed and shared macOS/Linux `NetworkErrorParityTests` pin Sendable and Equatable but intentionally not Hashable, Darwin POSIX raw/message output for common network failures (`EPIPE`, `EADDRINUSE`, `EADDRNOTAVAIL`, `ENETDOWN`, `ENETUNREACH`, `ENETRESET`, `ECONNABORTED`, `ECONNRESET`, `EISCONN`, `ENOTCONN`, `ESHUTDOWN`, `ETIMEDOUT`, `ECONNREFUSED`, `EHOSTDOWN`, and `EHOSTUNREACH`), `DNSServiceErrorType`/`OSStatus` `Int32` payloads, TLS `-9807` invalid-certificate-chain text, DNS unknown text, and Apple localizedDescription formatting. This is value-surface parity only; DNS/TLS transport behavior, connections, listeners, and socket behavior remain incomplete. |
...
| `CGPath.copy(using:)`, `CGPath(rect:transform:)`, `CGPath(roundedRect:cornerWidth:cornerHeight:transform:)`, and `CGMutablePath` rect/ellipse/line/quad/cubic recording | Usable | Transform-aware path copies and constructors preserve inspectable element data; rounded rectangles and ellipses record cubic curves instead of rectangle fallbacks. |
...
| Real event taps, keyboard state, pointer events, drawing APIs beyond imported Foundation/CoreFoundation geometry/path values | Incomplete | Required for CoreGraphics Parity. |

## Security

| API or function | Linux status | Notes |
| --- | --- | --- |
| `SecRandomCopyBytes(_:_:)` / `kSecRandomDefault` | Parity | Matches Apple's observed valid-count fill and zero-count status/mutation contract for Signal-style key-material generation. Random byte values intentionally differ. |
| `SecKeyCreateWithData(_:_:)`, `SecKeyCopyAttributes(_:)`, `SecKeyCopyExternalRepresentation(_:_:)` | Usable source compatibility | Imports non-empty key bytes into process-local `SecKey` objects and round-trips attributes/external bytes for Signal/libsignal-style imported key material. It does not perform Apple key validation or bind to native key handles or secure storage. Signing/verification is limited to deterministic compatibility signatures when EC key metadata permits it. |
| `SecKeyCreateRandomKey(_:_:)`, `SecKeyGeneratePair(_:_:_:)`, `SecKeyCopyPublicKey(_:)`, `SecKeyGetBlockSize(_:)` | Usable source compatibility | Synthesizes process-local private/public `SecKey` objects with requested key type/size/class/application-tag/capability metadata, fills random bytes through `SecRandomCopyBytes`, derives deterministic public-key bytes for reference wiring, can store permanent generated rows, and reports block size from external byte length. It does not perform native cryptographic key generation or validation, bind to native handles, or provide cryptographically valid signing, verification, or native/cryptographically valid key agreement. |
| `SecKeyIsAlgorithmSupported(_:_:_:)` and common `SecKeyAlgorithm` constants | Usable source compatibility | Common ECDSA, ECDH, and RSA algorithm constants plus the operation enum compile for Signal/libsignal source. The shim reports support for common ECDSA sign/verify, ECDH key exchange, and RSA PKCS1 encrypt/decrypt only when the `SecKey` type, class, and capability metadata allow the requested operation. `SecKeyCreateSignature`, `SecKeyVerifySignature`, and `SecKeyCopyKeyExchangeResult` are deterministic compatibility operations only, not native crypto or key validation. |
| `SecKeyCreateSignature(_:_:_:_:)`, `SecKeyVerifySignature(_:_:_:_:_:)` | Usable source compatibility | Deterministically signs and verifies common ECDSA SHA-256 message/digest payloads from matching process-local private/public `SecKey` metadata so Signal/libsignal-style source paths can exercise sign/verify control flow on Linux. This is not Apple DER ECDSA, not cryptographically secure, not native keychain-backed, and not suitable for production signatures. |
| `SecKeyCopyKeyExchangeResult(_:_:_:_:_:)`, `kSecKeyKeyExchangeParameterRequestedSize`, and `kSecKeyKeyExchangeParameterSharedInfo` | Usable source compatibility | Synthesizes deterministic, symmetric ECDH Standard/X963-SHA256 shared material from generated/imported EC key metadata and honors requested-size/shared-info parameters so Signal/libsignal-style handshake control flow can run on Linux. This is not real ECDH, not cryptographically secure, and not native keychain-backed key agreement. |
| `SecItemAdd(_:_:)` | Usable | Process-local item creation for generic-password, internet-password, and key-class rows. Detects duplicate identity across class, service, account, server, security domain, protocol, authentication type, port, path, access group, synchronizable attributes, application tag, application label, key class, key type, and key size, and can return attributes, data, object references, or persistent-reference `Data`. Key-class rows with stored key data synthesize process-local `SecKey` references when `kSecReturnRef` is requested. Not persistent or secure. |
| `SecItemCopyMatching(_:_:)` | Usable | Process-local matching with data, attributes, object reference, persistent reference, mixed data/persistent-reference dictionaries, internet endpoint filters, key-item metadata filters, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny` local-plus-synchronizable matching, not-found status, and `kSecMatchLimitAll` array results for current Signal/keychain storage contracts. Key-class data rows can return synthesized process-local `SecKey` references through `kSecReturnRef` or `kSecValueRef`. |
| `SecItemUpdate(_:_:)` | Usable | Updates matching process-local item values and attributes, preserving Apple-shaped duplicate and not-found status paths for generic-password, internet-password, and key-class identities. Updating key-class `kSecValueData` refreshes the synthesized process-local `SecKey` reference unless an explicit `kSecValueRef` is supplied. |
| `SecItemDelete(_:)` | Usable | Deletes matching process-local generic-password, internet-password, and key-class items, including delete-by-persistent-reference, endpoint-scoped deletes, and key metadata-scoped deletes, and returns not-found when no row matches. |
| `kSecReturnPersistentRef` / `kSecValuePersistentRef` | Usable | Apple-observed opaque `Data` handles round trip inside the process-local store for add, copy, mixed returns, and delete. Handles are not stable across processes or launches. |
| `SecItem` key-class rows (`kSecClassKey`, `kSecAttrApplicationTag`, `kSecAttrApplicationLabel`, `kSecAttrKeyClass`, `kSecAttrKeyType`, key size, permanence, and capability metadata) | Usable source compatibility | Stores, matches, and returns process-local key metadata/data and synthesized `SecKey` references for Signal/libsignal-style imported/generated key material. It does not validate native key encodings, bind to native key handles, perform native cryptographic key generation, cryptographically valid sign/verify, perform native/cryptographically valid key agreement, enforce access-control/Secure Enclave policy, or persist outside the process. |
| `SecAccessControlCreateWithFlags`, `SecAccessControlCreateFlags`, `kSecAttrAccessControl`, and `kSecUse*` authentication/query controls | Usable source compatibility | Creates, stores, and returns access-control metadata and treats data-protection/authentication use keys as query controls for Signal/libsignal-style keychain calls. Linux does not enforce access policy, biometric/passcode prompts, authentication context, or OS keychain selection. |
| `kSecRandomDefault`, `kSecClass*`, `kSecAttr*`, `kSecAttrSynchronizableAny`, `kSecValue*`, `kSecReturn*`, `kSecMatchLimit*`, `kSecUse*`, common internet-password protocol/authentication constants, key-item constants, common key algorithm/token/private/public-key constants, `errSec*` constants | Usable | Source-compatible constants for current Security, KeychainSwift-adjacent, and Signal-shaped storage contracts, including server/security-domain/protocol/authentication/port/path endpoint attributes, key application-tag/application-label/key-class/key-type/key-size/capability/token attributes, imported-key attribute dictionaries, and authentication/use query controls. |
...
| `KeychainSwift` and lower-level `Security` random/keychain storage | Upstream-shaped in-memory `KeychainSwift` UTF-8 string storage, raw data storage, single-byte bool storage, `getData(_:asReference:)`, deterministic reference handles, `allKeys`, delete, access-group/synchronizable namespaces, accessibility option metadata, result-code tracking, prefix read isolation, and namespace `clear()` behavior; `SecRandomCopyBytes` for Signal-shaped key-material generation; process-local `SecKeyCreateWithData`, `SecKeyCreateRandomKey`, `SecKeyGeneratePair`, `SecKeyCopyPublicKey`, `SecKeyGetBlockSize`, `SecKeyCopyAttributes`, `SecKeyCopyExternalRepresentation`, metadata-gated `SecKeyIsAlgorithmSupported`, deterministic ECDSA message/digest `SecKeyCreateSignature` and `SecKeyVerifySignature` compatibility, deterministic symmetric ECDH `SecKeyCopyKeyExchangeResult` compatibility with requested-size/shared-info parameters, and synthesized `SecKey` references for imported/generated key material; process-local `SecItemAdd`, `SecItemCopyMatching`, `SecItemUpdate`, and `SecItemDelete` generic-password, internet-password, and key-class rows with access-control metadata, authentication/use query controls, server/protocol/authentication/port/path endpoint separation, key-item application-tag/application-label/key-class/key-type/key-size/capability metadata, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and persistent-reference handles for account storage. | Secure OS keychain persistence, native access-control enforcement, true keychain sharing, iCloud synchronization, native item protection, cross-process behavior, native key validation/handles, native cryptographic key generation, cryptographically valid sign/verify, native/cryptographically valid key agreement, Secure Enclave behavior, and cryptographic equivalence beyond the valid-count random-byte contract. |
...
| `set(_:forKey:)` for `String`, `Data`, and `Bool` | Usable | Stores strings as UTF-8 bytes, data as raw bytes, and bools as upstream-shaped single bytes `[1]` / `[0]`. |
| `get(_:)`, `getData(_:asReference:)`, `getBool(_:)` | Usable | `get` decodes UTF-8 bytes, `getData` returns either stored bytes or deterministic reference handles, and `getBool` follows upstream first-byte behavior. |
| `allKeys` | Usable | Returns sorted keys for the current access-group/synchronizable namespace, including prefixed keys. |
| `delete(_:)` | Usable | Removes one prefix-scoped key, returns false for missing keys, and updates `lastResultCode`. |
| `clear()` | Usable | Removes all values for the current access-group/synchronizable namespace, matching upstream behavior that does not restrict the deletion to `keyPrefix`. |
...
| `lastResultCode` | Usable | Tracks success, not-found, and invalid-encoding statuses for the current process-local operations. |
...
| Lower-level `Security` `SecKeyCreateWithData`, `SecKeyCopyAttributes`, `SecKeyCopyExternalRepresentation`, and synthesized `SecKey` references | Usable source compatibility | Supports process-local imported-key byte/attribute round-trips and object references for Signal/libsignal-style key material. It does not validate native key encodings or bind to native key handles. Signing/verification is limited to deterministic compatibility signatures when EC key metadata permits it. |
| Lower-level `Security` `SecKeyCreateRandomKey`, `SecKeyGeneratePair`, `SecKeyCopyPublicKey`, `SecKeyGetBlockSize`, and `SecKeyIsAlgorithmSupported` | Usable source compatibility | Synthesizes process-local generated private/public key references with requested metadata, stores permanent rows, derives stable public-key bytes, reports byte-length block sizes, and answers common ECDSA/ECDH/RSA support checks from key type/class/capability metadata for Signal/libsignal-style source compatibility. It is not native cryptographic key generation and does not provide cryptographically valid sign/verify, encrypt, decrypt, or native/cryptographically valid key agreement. |
| Lower-level `Security` `SecKeyCreateSignature` and `SecKeyVerifySignature` | Usable source compatibility | Deterministically signs and verifies common ECDSA SHA-256 message/digest payloads for matching generated/imported key metadata, giving Signal/libsignal-style source a Linux control-flow target. This is not Apple DER ECDSA, not cryptographically secure, and not suitable for production signatures. |
| Lower-level `Security` `SecKeyCopyKeyExchangeResult` | Usable source compatibility | Synthesizes deterministic, symmetric ECDH Standard/X963-SHA256 shared material from process-local EC key metadata and honors requested-size/shared-info parameters so Signal/libsignal-style key-agreement control flow can run on Linux. This is not real ECDH, not cryptographically secure, and not native keychain-backed key agreement. |
| Lower-level `Security` `SecItem` generic-password, internet-password, and key-class add/copy/update/delete | Usable | Supports process-local keychain-shaped rows, access-control metadata, authentication/use query controls, server/protocol/authentication/port/path endpoint separation, key-item application-tag/application-label/key-class/key-type/key-size/capability metadata, synthesized `SecKey` object references, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and persistent-reference handles Signal-style libraries commonly source-target before a secure backend is attached. |
| Native secure persistence, access-control enforcement, keychain sharing, synchronization, cross-process lookup, native key validation/handles, native cryptographic key generation, cryptographically valid sign/verify, native/cryptographically valid key agreement, Secure Enclave policy | Incomplete | Required before claiming KeychainSwift or Security keychain parity. |
