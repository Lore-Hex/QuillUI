...
| `Security` | `SecRandomCopyBytes`, process-local `SecItemAdd`, `SecItemCopyMatching`, `SecItemUpdate`, `SecItemDelete` generic-password and internet-password rows, persistent-reference returns/lookups/deletes, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, server/security-domain/protocol/authentication/port/path endpoint identity, and current keychain constants. | Native secure keychain persistence/access control, OS-enforced keychain sharing, real keychain synchronization, cross-process lookup, certificate parsing, policy evaluation, platform trust store, and Secure Transport parity are incomplete. |
...
| `kSecRandomDefault`, `kSecClass*`, `kSecAttr*`, `kSecAttrSynchronizableAny`, `kSecValue*`, `kSecReturn*`, `kSecMatchLimit*`, common internet-password protocol/authentication constants, `errSec*` constants | Usable | Source-compatible constants for current Security, KeychainSwift-adjacent, and Signal-shaped storage contracts, including server/security-domain/protocol/authentication/port/path endpoint attributes. |
...
| Native secure keychain persistence, access control, OS-enforced keychain sharing, real keychain synchronization, cross-process lookup, certificate parsing, policy evaluation, platform trust store, Secure Transport parity | Incomplete | Current keychain rows are process-local compatibility storage, and current trust fallback must not be treated as production TLS trust. |
...
| Lower-level `Security` `SecItem` generic-password and internet-password add/copy/update/delete | Usable | Supports process-local keychain-shaped rows, server/protocol/authentication/port/path endpoint separation, access-group namespace filters, synchronizable filters, `kSecAttrSynchronizableAny`, and persistent-reference handles Signal-style libraries commonly source-target before a secure backend is attached. |
