...
| `Network` | `IPv4Address`, `IPv6Address`, `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWEndpoint.Host`, `NWEndpoint.Port` parsing/known constants/properties/debug text, and common `NWEndpoint` value descriptions now have Apple-checked `Parity` rows for current tested contracts. | Path monitoring, constructed interfaces, connections, listeners, DNS, TLS, UDP/TCP, and real path probing are incomplete or fallback-only. |
...
| `NWEndpoint.Host.init(_:)` / direct host cases / `description` / `debugDescription` | Parity | Classifies Apple-matching IPv4 literals, IPv6 literals, and DNS names through the parity-tested address value parsers, mirrors direct `.name`, `.ipv4`, and `.ipv6` case descriptions, and exposes Apple-matching debug text. |
| `NWEndpoint.Port.init?(String)` / `init?(rawValue:)` / integer literal / `rawValue` / `String(describing:)` / `debugDescription` | Parity | Apple-checked port parsing covers leading C whitespace, plus signs, `-0`, decimal-only input, UInt16 bounds, trailing-character rejection, failable raw-value construction, integer literals, and decimal debug/string-describing output. Direct `.description` is not an Apple public API for this value and is intentionally not cloned. |
| `NWEndpoint.Port.any` / `.ssh` / `.smtp` / `.http` / `.pop` / `.imap` / `.https` / `.imaps` / `.socks` | Parity | Apple-checked well-known port constants expose raw values `0`, `22`, `25`, `80`, `110`, `143`, `443`, `993`, and `1080`, with matching debug/string-describing output. |
| `NWEndpoint.hostPort`, `service`, and `unix` value `description` / `debugDescription` | Parity | Apple-checked host-port, Unix path, and valid DNS-SD service string/debug descriptions, including IPv6 host-port separator behavior and service type/domain dot normalization. |
