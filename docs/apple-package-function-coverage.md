...
| `SecKeyIsAlgorithmSupported(_:_:_:)` and common `SecKeyAlgorithm` constants | Compile-compatible fallback | Common ECDSA, ECDH, and RSA algorithm constants plus the operation enum compile for Signal/libsignal source. The fallback reports unsupported until signing, verification, encryption, decryption, and key agreement are implemented. |
...
| Lower-level `Security` `SecKeyCreateRandomKey`, `SecKeyGeneratePair`, `SecKeyCopyPublicKey`, and `SecKeyGetBlockSize` | Usable source compatibility | Synthesizes process-local generated private/public key references with requested metadata, stores permanent rows, derives stable public-key bytes, and reports byte-length block sizes for Signal/libsignal-style source compatibility. It is not native cryptographic key generation and does not sign, verify, or perform key agreement. |
