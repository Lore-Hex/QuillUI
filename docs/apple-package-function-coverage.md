...
| `IPv4Address.init?(String)` / `init?(Data)` / `String(describing:)` / `debugDescription` | Parity | Apple-checked IPv4 parser covers strict whitespace rejection, legacy 1-4 component forms, octal and hex components, 32-bit wraparound, 4-byte data validation, scoped interface literals through resolved OS interface names, and canonical dotted-decimal string/debug output. Linux also exposes `.description` as a compatibility alias, but direct `.description` is not an Apple public API for this value. |
...
| `IPv4Address.interface` / `isLoopback` / `isLinkLocal` / `isMulticast` | Parity | Apple-checked nil interface for unscoped literal/data construction, scoped literal interface preservation through resolved OS interface names, plus exact loopback, `169.254/16` link-local, and `224/4` multicast classification for tested edge cases. |
| `IPv6Address.init?(String)` / `init?(Data)` / `String(describing:)` / `debugDescription` | Parity | Apple-checked IPv6 parser covers `inet_pton` literal acceptance, whitespace rejection, 16-byte data validation, scoped interface literals through resolved OS interface names, and canonical `inet_ntop` string/debug output. Linux also exposes `.description` as a compatibility alias, but direct `.description` is not an Apple public API for this value. |
