...
2. **SCN scene-graph shim authored; fixtures compile** — ✅ DONE for the
   ...
   light/camera/parenting/traversal/replacement/cloning/local and world transforms/bounding boxes/runAction/`look(at:)`), `SCNGeometry` + the
...
- [x] Rung 2 (fixtures): SceneKit scene-graph shim authored with node parenting/traversal/replacement/cloning/local and world transforms/bounding boxes; QuillSolarSystem + QuillMoleculeViewer compile
