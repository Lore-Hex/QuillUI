...
- `quill-source-lower`, with `scripts/lower-swiftdata-for-quilldata.sh` as a compatibility wrapper, which creates a generated source copy that lowers `@Model`, `@Transient`, `#Predicate<T>`, and supported `@Relationship` inverse hooks into QuillData-compatible Swift without editing app sources.
...
- Foundation `#Predicate` can trap before QuillData sees it when built against plain class models outside SwiftData's macro/runtime path. The source lowerer rewrites those call sites to `QuillPredicate`; direct hand ports can also use `QuillPredicate` or `FetchDescriptor(filter:)`.
- Relationships currently encode as model fields; delete rules are not schema-native yet. In-memory inverse maintenance is source-lowered for supported non-optional to-many inverse pairs.
