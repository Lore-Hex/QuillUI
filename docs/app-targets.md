...
Linux window sizing note: WireGuard's shared default window width now matches the Linux minimum width, so GTK and Qt start from the same unclamped dimensions before either host applies backend-specific window constraints.
