...
A process-local `Security` surface now covers `SecRandomCopyBytes` for Signal-style key-material generation plus `SecItem` generic-password add/copy/update/delete, duplicate detection, returned attributes/data, and match-all queries so future libsignal/account storage code has a Linux source target. Native secure persistence, access control, access groups, synchronization, and cross-process keychain behavior remain blockers.
