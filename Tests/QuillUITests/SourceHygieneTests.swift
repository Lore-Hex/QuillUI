...
struct SourceHygieneTests {
    ...
    func qtManifestAvoidsPkgConfigProhibitedFlagWarnings() throws {
        ...
        #expect(manifest.occurrences(of: "swiftSettings: quillLegacyMainActorSwiftSettings") == 3)
        #expect(manifest.occurrences(of: "\"-strict-concurrency=minimal\", \"-default-isolation\", \"MainActor\"") == 1)
...
"""))
    ...
    func linuxAppleCompatibilityShimsAvoidGeneratedAppWarnings() throws {
        ...
        #expect(captureSurface.contains("public class AVCaptureSession: @unchecked Sendable"))
        #expect(captureSurface.contains("quillV4L2StartIfAvailable()"))
    ...
    func linuxBackendFacadesScriptsAndWorkflowsStayBackendNeutral() throws {
        ...
        #expect(backendScript.contains("reset_quill_chat_completions_panel_selection()"))
        #expect(backendScript.contains("QUILLUI_BACKEND_COMPLETIONS_RESET_CLICK_X:-$((window_x + 190))"))
        #expect(backendScript.contains("QUILLUI_BACKEND_COMPLETIONS_RESET_CLICK_Y:-$(quill_chat_mac_reference_history_row_y recent-transcript)"))
        #expect(backendScript.contains("QUILLUI_BACKEND_COMPLETIONS_RESET_SETTLE_SLEEP:-0.2"))
        #expect(backendScript.contains("QUILLUI_BACKEND_COMPLETIONS_RESET_SLEEP:-0.6"))
        #expect(backendScript.contains("quill_chat_completions_probe_sequence=$((quill_chat_completions_probe_sequence + 1))"))
        #expect(backendScript.contains("quill-chat-completions-panel-probe-${INTERACTION_MODE}-${INTERACTION_ATTEMPT}-${quill_chat_completions_probe_sequence}.png"))
        #expect(backendScript.contains("interaction-check: Quill Chat Completions open attempt"))
        #expect(backendScript.contains("interaction-check: click Quill Chat Completions toggle reset at"))
        ...
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key i"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key h"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key v"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key bracketright"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key 0"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key b"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key space"))
        #expect(solderScopeSmoke.contains("DISPLAY=\"$DISPLAY_ID\" xdotool key Escape"))
