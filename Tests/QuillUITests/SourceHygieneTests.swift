...
"""))
    ...
    func linuxSwiftUICompatibilityExposesAccessibilityModifiers() throws {
        ...
        #expect(desktopInteraction.occurrences(of: "public var body: Content { content }") == 4)
        #expect(!desktopInteraction.contains("backend-rendered primitive"))
        #expect(!desktopInteraction.contains("fatalError(\"Accessibility"))
        ...
        #expect(compatibilityModifiers.occurrences(of: "public var body: Content { content }") == 4)
        #expect(!compatibilityModifiers.contains("backend-rendered primitive"))
        #expect(!compatibilityModifiers.contains("fatalError(\"QuillCompatibility"))
    ...
    func semanticColorsHaveSinglePlatformColorOwner() throws {
        ...
        #expect(foundation.contains("public override func isEqual(_ object: Any?) -> Bool"))
        #expect(foundation.contains("public override var hash: Int"))
        #expect(uiKit.contains("public final class UIFont: NSObject, NSSecureCoding"))
        #expect(uiKit.contains("public static var supportsSecureCoding: Bool { true }"))
    ...
    func linuxAppleCompatibilityShimsAvoidGeneratedAppWarnings() throws {
        ...
        #expect(captureTests.contains("realtimeWriterCanStrideAutomaticCaptureFrames"))
    ...
    func linuxSwiftUICompatibilityExtensionsHaveOneCanonicalModule() throws {
        ...
        let qtBackend = try String(
            contentsOf: root.appendingPathComponent("Sources/BackendQt/QtBackend.swift"),
            encoding: .utf8
        )
        ...
        #expect(swiftUIPlatformSurface.contains("QuillUIApplicationDelegateFactory.Type"))
        #expect(swiftUIPlatformSurface.contains("didFinishLaunchingWithOptions: nil"))
        #expect(swiftUIPlatformSurface.contains("SwiftOpenUIAppLifecycle.registerPostInitialization"))
        #expect(swiftOpenUIApp.contains("public enum SwiftOpenUIAppLifecycle"))
        #expect(swiftOpenUIApp.contains("postInitializationCallbacks.removeAll(keepingCapacity: true)"))
        #expect(gtkBackend.contains("let instance = A()\n                SwiftOpenUIAppLifecycle.appDidInitialize()"))
        #expect(qtBackend.contains("SwiftOpenUIAppLifecycle.appDidInitialize()\n            qtRenderScene(instance.body"))
        #expect(!swiftUIPlatformSurface.contains("UIApplicationDelegateAdaptor has no Linux runtime delegate instance"))
    ...
    func linuxBackendFacadesScriptsAndWorkflowsStayBackendNeutral() throws {
        ...
        #expect(backendProducts.contains("verify_product=\"quill-chat-linux-mac-reference-completions-panel-visible\""))
        ...
        #expect(backendScript.contains("quill_chat_settings_click_x()"))
        #expect(backendScript.contains("QUILLUI_BACKEND_SETTINGS_CLICK_X:-$((window_x + 88))"))
        #expect(backendScript.contains("QUILLUI_BACKEND_SETTINGS_CLICK_Y:-$((window_y + window_height - 44))"))
        #expect(!backendScript.contains("QUILLUI_BACKEND_SETTINGS_CLICK_Y:-$((window_y + window_height - 22))"))
        #expect(backendScript.contains("quill_chat_alert_settings_click_y()"))
        #expect(backendScript.contains("window_height - 355"))
        ...
        #expect(backendScript.contains("quill-chat-linux-mac-reference-completions-panel-visible"))
        #expect(backendScript.contains("dismiss_quill_chat_completions_overlay_if_visible()"))
        #expect(backendScript.contains("QUILLUI_BACKEND_COMPLETIONS_CLOSE_CLICK_X"))
        ...
        #expect(backendScript.contains("quill_chat_mac_reference_composer_click_target()"))
        #expect(backendScript.contains("click_quill_chat_composer()"))
        ...
        #expect(backendScript.contains("quill-chat-composer"))
        ...
        #expect(screenshotVerifier.contains("minimum_mean = 180 if solderscope_launch_product else 1000"))
        ...
        #expect(screenshotVerifier.contains("def generic_qt_splitter_pixel"))
        #expect(screenshotVerifier.contains("def generic_qt_splitter_column_score"))
        #expect(screenshotVerifier.contains("key=lambda x: generic_qt_splitter_column_score"))
        ...
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE=1"))
        ...
        #expect(solderScopeSmoke.contains("QUILL_AVFOUNDATION_REALTIME_RECORDING_FRAME_STRIDE=\"${QUILL_AVFOUNDATION_REALTIME_RECORDING_FRAME_STRIDE:-6}\""))
        ...
        #expect(solderScopeSmoke.contains("local recording_driver=\"${QUILLUI_SOLDERSCOPE_RECORDING_DRIVER:-shortcut}\""))
        ...
        #expect(solderScopeSmoke.contains("local recording_stop_driver=\"${QUILLUI_SOLDERSCOPE_RECORDING_STOP_DRIVER:-$recording_driver}\""))
        ...
        #expect(solderScopeSmoke.contains("quillui_solderscope_alternate_key_driver()"))
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_START_RETRY_KEY_DRIVER:-auto"))
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_STOP_RETRY_KEY_DRIVER:-auto"))
        #expect(solderScopeSmoke.contains("recording_start_retry_key_driver"))
        #expect(solderScopeSmoke.contains("recording_stop_retry_key_driver"))
        #expect(solderScopeSmoke.contains("shortcut $label $recording_key_driver key r"))
        ...
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_START_FALLBACK_RETRY_INTERVAL_TICKS:-8"))
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_START_INDICATOR_PROBE_INTERVAL_TICKS:-1"))
        ...
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_STOP_FALLBACK_RETRY_INTERVAL_TICKS:-8"))
        ...
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_START_RETRY_INTERVAL_TICKS:-8"))
        ...
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_RECORDING_STOP_RETRY_INTERVAL_TICKS:-8"))
        #expect(solderScopeSmoke.contains("should_retry_stop == 1 && recording_saved_count <= recording_saved_before"))
        ...
        #expect(solderScopeSmoke.contains("SOLDERSCOPE_LAST_VISIBLE_SCREENSHOT=\"${SCREENSHOT_PATH%.png}-last-visible.png\""))
        #expect(solderScopeSmoke.contains("cp -f \"$frame_probe_path\" \"$SOLDERSCOPE_LAST_VISIBLE_SCREENSHOT\""))
        #expect(solderScopeSmoke.contains("rm -f \"$SOLDERSCOPE_LAST_VISIBLE_SCREENSHOT\""))
        #expect(solderScopeSmoke.contains("quillui_solderscope_copy_verified_final_frame()"))
        #expect(solderScopeSmoke.contains("\"$OUTPUT_DIR\"/quill-solderscope-*-last-visible.png"))
        #expect(solderScopeSmoke.contains("\"$OUTPUT_DIR\"/quill-solderscope-*-snapshot-settled.png"))
        #expect(solderScopeSmoke.contains("\"$ROOT_DIR/scripts/verify-backend-screenshot.py\" \"$candidate\" \"$VERIFY_PRODUCT\""))
        #expect(solderScopeSmoke.contains("SolderScope interaction smoke: using verified final frame $candidate"))
        #expect(solderScopeSmoke.contains("quillui_solderscope_copy_verified_final_frame \"$SCREENSHOT_PATH\""))
        #expect(solderScopeSmoke.contains("quillui_solderscope_wait_for_visible_frame_with_retry \\\n      \"after recording\" \"$settled_recording_screenshot\" \"$window_id\" \"$window_width\" \"$window_height\""))
        #expect(solderScopeSmoke.contains("QUILLUI_SOLDERSCOPE_REQUIRE_FRAME_AFTER_RECORDING:-0"))
        #expect(solderScopeSmoke.contains("continuing recording verification despite missing post-recording frame"))
        ...
        #expect(solderScopeSmoke.contains("record-stop-fallback-retry"))
    ...
    func quillDesktopSplitLayoutMirrorsMacReferenceTitlebarChrome() throws {
        ...
        #expect(controls.contains("VStack(alignment: isUserMessage ? .trailing : .leading, spacing: 2)"))
        #expect(controls.contains("alignment: isUserMessage ? .trailing : .leading"))
        ...
        #expect(messageHoverActions.contains("gtk_widget_set_can_target(contentWidget, 1)"))
        #expect(messageHoverActions.contains("gtk_widget_set_halign(contentWidget, isUserMessage ? GTK_ALIGN_END : GTK_ALIGN_START)"))
        #expect(messageHoverActions.contains("gtk_widget_set_can_target(actionWidget, 1)"))
        ...
        #expect(messageHoverActions.contains("gtk_widget_set_opacity(actionWidget, 1)"))
        ...
        #expect(controls.contains("extension QuillMessageList: GTKRenderable"))
        #expect(controls.contains("public func gtkCreateWidget() -> OpaquePointer"))
        #expect(controls.contains("let scrolled = gtk_scrolled_window_new()!"))
        #expect(controls.contains("gtk_scrolled_window_set_policy(OpaquePointer(scrolled), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC)"))
        #expect(controls.contains("gtk_scrolled_window_set_child(OpaquePointer(scrolled), stack)"))
        #expect(controls.contains("gtk_overlay_set_child(OpaquePointer(overlay), scrolled)"))
        #expect(controls.contains("quillGTKScheduleMessageListBottomScroll(scrolled)"))
        #expect(controls.contains("private final class QuillGTKMessageListScrollState"))
        #expect(controls.contains("gtk_adjustment_set_value(adjustment, max(lower, upper - pageSize))"))
        #expect(controls.contains("remainingAppliedTicks: Int = 600, remainingTotalTicks: Int = 900"))
        #expect(controls.contains("g_timeout_add(16, { userData -> gboolean in"))
...
"""))
    ...
    func gtkQuillPaintHooksCoverButtonAndTextInputChrome() throws {
        ...
        #expect(renderer.contains("private final class GTKTextBindingUpdateSource"))
        #expect(renderer.contains("weak var ownerHost: GTKViewHost?"))
        #expect(renderer.contains("private final class GTKTextBindingIdleUpdate"))
        #expect(renderer.contains("pending.source !== source"))
        #expect(renderer.contains("func gtkFlushPendingTextBindingUpdate(for ownerHost: GTKViewHost? = nil)"))
        #expect(gtkViewHost.contains("gtkFlushPendingTextBindingUpdate(for: self)"))
        ...
        #expect(renderer.contains("gtkScheduleTextBindingUpdate(updateSource, value: newText)"))
        ...
        #expect(renderer.contains("return { deferredAction.invoke(()) }"))
        #expect(renderer.contains("return { value in deferredAction.invoke(value) }"))
        ...
        #expect(patcher.contains("private final class GTKTextBindingUpdateSource"))
        #expect(patcher.contains("gtkFlushPendingTextBindingUpdate(for: self)"))
        #expect(patcher.contains("replace_with_canonical_section"))
        ...
        #expect(gtkTextField.contains("gtk_widget_set_can_target(overlay, 1)"))
        ...
        #expect(gtkToggle.contains("let chromeButton = gtk_button_new()!"))
        #expect(gtkToggle.contains("gtk_button_set_child(quillToggleButtonPointer(chromeButton), chrome)"))
        #expect(gtkToggle.contains("gtk_widget_set_can_target(button, 1)"))
        #expect(gtkToggle.contains("gtk_widget_set_can_target(control, 0)"))
        #expect(gtkToggle.contains("installQuillToggleButtonAction(chromeButton, control: controlWidget, isSwitch: isSwitch)"))
        #expect(gtkToggle.contains("connectQuillToggleButtonRedrawSignals(button: chromeButton, chromeBox: chromeBox)"))
        #expect(gtkToggle.contains("connectQuillToggleControlSensitivity(control: controlWidget, button: chromeButton, chromeBox: chromeBox)"))
        #expect(gtkToggle.contains("private func installQuillToggleButtonAction"))
        #expect(gtkToggle.contains("private func quillToggleButtonPointer"))
        #expect(gtkToggle.contains("button.quill-paint-toggle-button"))
        #expect(gtkToggle.contains("private var lastActivation = Date.distantPast"))
        #expect(gtkToggle.contains("guard now.timeIntervalSince(lastActivation) > 0.15 else { return }"))
        #expect(gtkToggle.contains("guard gtk_widget_get_sensitive(control) != 0 else { return }"))
        #expect(gtkToggle.contains("gtk_swift_gesture_single_set_button(gesture, 1)"))
        #expect(gtkToggle.contains("gtk_swift_add_capture_gesture(button, gesture)"))
        #expect(gtkToggle.contains("private func installQuillToggleActivationGesture"))
        #expect(gtkToggle.contains("gtk_swift_add_capture_gesture(widget, gesture)"))
    ...
    func uiViewRepresentableTextBindingsLowerToSwiftOpenUITextEditor() throws {
        ...
        #expect(representable.contains("private final class QuillUIViewControllerRepresentableMount"))
        #expect(representable.contains("@StateObject private var mount: QuillUIViewControllerRepresentableMount<R>"))
        #expect(representable.contains("let controller = mount.update(with: representable)"))
        #expect(representable.contains("coordinatorRetainer: mount.coordinator"))
        ...
        #expect(representable.contains("private final class QuillUIViewRepresentableMount"))
        #expect(representable.contains("@StateObject private var mount: QuillUIViewRepresentableMount<R>"))
        #expect(representable.contains("func hostedContent(updatingWith representable: R) -> AnyView?"))
        #expect(representable.contains("mount.hostedContent(updatingWith: representable)"))
        ...
        #expect(representable.contains("static func dismantleUIView(_ uiView: UIViewType, coordinator: Coordinator)"))
        #expect(representable.contains("static func dismantleUIViewController("))
    ...
    func gtkNavigationStackPreservesEmptyDestinationNavigationTitles() throws {
        ...
        #expect(navigation.contains("gtkSetNavigationPageInteractive(current.widget, false)"))
        #expect(navigation.contains("gtkSetNavigationPageInteractive(widget, true)"))
        #expect(navigation.contains("gtkSetNavigationPageInteractive(removed.widget, false)"))
        #expect(navigation.contains("gtkSetNavigationPageInteractive(previous.widget, true)"))
        #expect(navigation.contains("gtkSetNavigationPageInteractive(rootWidget, true)"))
        #expect(navigation.contains("gtkSwiftNavigationPageInteractivityMarker"))
        #expect(navigation.contains("gtkSwiftNavigationPageInactiveValue"))
        #expect(navigation.contains("g_object_set_data(\n        gobject,\n        gtkSwiftNavigationPageInteractivityMarker"))
        #expect(navigation.contains("gtk_widget_set_can_target(widget, isInteractive ? 1 : 0)"))
        ...
        #expect(navigation.contains("let windowID = getCurrentEnvironment().windowID"))
        #expect(navigation.contains("weak var owner: GTKViewHost?"))
        #expect(navigation.contains("$0.belongs(toWindowID: windowID, owner: owner)"))
        ...
        #expect(renderer.contains("gtkWidgetIsInsideInactiveNavigationPage"))
        #expect(renderer.contains("guard !gtkWidgetIsInsideInactiveNavigationPage(widget) else { return nil }"))
        #expect(renderer.contains("button root skipped inactive navigation page"))
        ...
        #expect(renderer.contains("private var gtkEnvironmentObjectTypeCounters"))
        #expect(renderer.contains("gtkEnvironmentObjectScope(for: ObjectType.self)"))
        #expect(renderer.contains("let stateNamespace = gtkClaimStateIdentityNamespace(\"GeometryReader\")"))
        #expect(renderer.contains("var environment = capturedEnvironment"))
        #expect(viewHost.contains("[ObjectIdentifier: EnvironmentObjectCapture]"))
        #expect(viewHost.contains("endScopedEnvironmentReadTracking()"))
        #expect(environment.contains("public struct EnvironmentObjectCapture"))
        #expect(environment.contains("scope: capture.scope"))
        ...
        #expect(renderTests.contains("testNavigationActionTargetSurvivesViewHostReplacementWithinWindow"))
        #expect(renderTests.contains("testNavigationActionTargetDoesNotCrossWindows"))
        #expect(renderTests.contains("testNavigationActionTargetFallsBackToViewHostWithoutWindowIdentity"))
        ...
        #expect(renderTests.contains("testRebuiltMenuActionRetainsSiblingScopedEnvironmentObject"))
        #expect(renderTests.contains("testGeometryReaderDeferredRenderPreservesEnvironmentBinding"))
        ...
        #expect(patcher.contains("gtkEnvironmentObjectTypeCounters"))
        #expect(patcher.contains("endScopedEnvironmentReadTracking"))
        #expect(patcher.contains("gtkResumeViewHostLifecycleForVisibleSubtree"))
        #expect(patcher.contains("gtkSetNavigationPageInteractive(current.widget, false)"))
        #expect(patcher.contains("gtkSetNavigationPageInteractive(widget, true)"))
        #expect(patcher.contains("gtkSetNavigationPageInteractive(removed.widget, false)"))
        #expect(patcher.contains("gtkSetNavigationPageInteractive(previous.widget, true)"))
        #expect(patcher.contains("gtkSetNavigationPageInteractive(rootWidget, true)"))
        #expect(patcher.contains("gtk_widget_set_can_target(widget, isInteractive ? 1 : 0)"))
        ...
        #expect(patcher.contains("let windowID = getCurrentEnvironment().windowID"))
        #expect(patcher.contains("SwiftOpenUI GTK window-scoped navigation registry shape was not recognized"))
    ...
    func gtkPlainButtonStyleSuppressesPlatformChrome() throws {
        ...
        for source in [renderer, patcher] {
            ...
            #expect(source.contains("source: gtkButtonDebugSource(\"gesture\", widget: context.widget)"))
            #expect(source.contains("phase: .pointerPress"))
            ...
            #expect(source.contains("private typealias GTKVisualButtonActionCandidate"))
            #expect(source.contains("private func gtkPreferredVisualButtonActionCandidate"))
            #expect(source.contains("if proposed.depth > current.depth { return proposed }"))
            #expect(source.contains("if proposed.depth == current.depth && proposed.area < current.area { return proposed }"))
            #expect(source.contains("gtkVisualButtonActionCandidateAtRootPoint"))
    ...
    func gtkMainActorBodyRenderingDoesNotReturnRawPointersThroughAssumeIsolated() throws {
        ...
        #expect(renderer.contains("gtkAssumeMainActorIsolated { gtkRenderView(buildContent()) }"))
        #expect(renderer.contains("gtkAssumeMainActorIsolated { gtkDescribeView(buildContent()) }"))
        ...
        #expect(!renderer.contains("MainActor.assumeIsolated { gtkRenderView(buildContent()) }"))
        #expect(!renderer.contains("MainActor.assumeIsolated { gtkDescribeView(buildContent()) }"))
        #expect(!navigation.contains("return MainActor.assumeIsolated { gtkRenderToolbarWidgets(from: view.body) }"))
    }

    @Test("GTK window roots rebuild deferred scene content and isolate nested lifecycle")
    func gtkWindowRootsRebuildDeferredSceneContentAndIsolateNestedLifecycle() throws {
        let app = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/App/App.swift")
        let sizing = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/App/WindowSizing.swift")
        let backend = try packageSource("third_party/SwiftOpenUI/Sources/Backend/GTK4/Rendering/GTK4Backend.swift")
        let renderer = try packageSource("third_party/SwiftOpenUI/Sources/Backend/GTK4/Rendering/GTKRenderer.swift")
        let descriptorTree = try packageSource("third_party/SwiftOpenUI/Sources/Backend/GTK4/Rendering/GTK4DescriptorTree.swift")
        let windowTests = try packageSource("third_party/SwiftOpenUI/Tests/SwiftOpenUITests/WindowSceneTests.swift")
        let renderTests = try packageSource("third_party/SwiftOpenUI/Tests/BackendTests/GTK4Tests/GTK4RenderTests.swift")
        let patcher = try packageSource("scripts/patch-swiftopenui-gtk-css.sh")

        #expect(app.contains("public let quillContentFactory: () -> Content"))
        #expect(app.contains("@ViewBuilder content: @escaping () -> Content"))
        #expect(app.contains("self.quillContentFactory = quillContentFactory ?? { content }"))
        #expect(sizing.occurrences(of: "quillContentFactory: quillContentFactory") == 6)
        #expect(backend.contains("contentFactory: quillContentFactory"))
        #expect(backend.contains("contentProvider: contentFactory"))
        #expect(renderer.contains("let buildContent = contentProvider ?? { view }"))
        #expect(renderer.occurrences(of: "buildContent()") == 2)

        #expect(descriptorTree.contains("case statefulLifecycleScope"))
        #expect(descriptorTree.contains("kind = .statefulLifecycleScope"))
        #expect(descriptorTree.contains("if node.descriptor.kind == .statefulLifecycleScope"))
        #expect(windowTests.contains("testStartupWindowGroupReevaluatesOriginalContentFactory"))
        #expect(windowTests.contains("testStartupWindowGroupContentFactorySurvivesSizingModifiers"))
        #expect(renderTests.contains("testWindowRootReevaluatesContentProviderAfterAppObservableMutation"))

        #expect(patcher.contains("original_observable = observable_path.read_text()"))
        #expect(patcher.contains("if observable != original_observable:"))
        #expect(patcher.contains("original_bindable = bindable_path.read_text()"))
        #expect(patcher.contains("if bindable != original_bindable:"))
        #expect(patcher.contains("text = text.replace('css += \" object-fit: contain;\"', 'css += \"\"')"))
        #expect(!patcher.contains("s/css \\+= \" object-fit: contain;\"/css += \"\"/g"))
        #expect(patcher.contains("deferred WindowGroup property"))
        #expect(patcher.contains("GTK root deferred content provider"))
        #expect(patcher.contains("SwiftOpenUI stateful descriptor lifecycle ownership shape was not recognized"))
    }
    ...
    func gtkPatcherPreservesFixedFrameAndListViewportSizingContracts() throws {
        ...
        #expect(patcher.contains("gtkInstallScrollViewCrossAxisFill(on: scrolled, child: listBox, fillWidth: true, fillHeight: true)"))
        ...
        #expect(renderer.contains("private func gtkSheetPresentationEnvironment("))
        #expect(renderer.contains("env.isPresentedInSheet = true"))
        #expect(renderer.occurrences(of: "gtkSheetPresentationEnvironment(") >= 7)
        #expect(!renderer.contains("env.dismiss = DismissAction(handler: dismissAction, debugName: \"gtk sheet"))
        #expect(patcher.contains("sheet_presentation_environment = '''private func gtkSheetPresentationEnvironment("))
        #expect(patcher.contains("env.isPresentedInSheet = true"))
        #expect(patcher.contains("gtkSheetPresentationEnvironment("))
        #expect(renderTests.contains("testRootOverlaySheetNavigationStackUsesInlineSheetChrome"))
        #expect(renderTests.contains("GTKSheetNavigationChromeProbeView"))
        ...
        #expect(patcher.contains("for debug_name in ["))
        #expect(patcher.contains("env = gtkSheetPresentationEnvironment("))
        ...
        #expect(patcher.contains("gtk_widget_set_can_target(panel, 1)"))
        #expect(patcher.contains("private func gtkRootSheetLayerOccludesRootPoint("))
        #expect(patcher.contains("guard let layerRoot = gtk_widget_get_root(layer),"))
        #expect(patcher.contains("gpointer(layerRoot) == gpointer(root)"))
        #expect(patcher.contains("gtkWidgetIsDescendant(excluded, of: layer)"))
        #expect(patcher.contains("button global dispatch skipped root sheet root@"))
        #expect(patcher.contains("button root skipped root sheet root@"))
        #expect(patcher.contains("tap gesture global dispatch skipped root sheet root@"))
        #expect(patcher.contains("tap gesture root skipped root sheet root@"))
        #expect(patcher.contains("list row global dispatch skipped root sheet root@"))
        #expect(patcher.contains("listbox-root skipped root sheet root@"))
        #expect(patcher.contains("list row tap skipped root sheet root@"))
        ...
        #expect(patcher.contains("gtkSheetPointTargetsControl(root: root, rootX: rootX, rootY: rootY)"))
        #expect(patcher.contains("sheet focus bridge skipped control root@"))
        #expect(patcher.contains("gtk_swift_widget_is_scale(widget)"))
        #expect(renderer.contains("gtkSheetPointTargetsControl(root: root, rootX: rootX, rootY: rootY)"))
        #expect(renderer.contains("sheet focus bridge skipped control root@"))
        #expect(renderer.contains("gtk_swift_widget_is_scale(widget)"))
        #expect(patcher.contains("sheet focus bridge editable root@"))
    ...
    func vendoredGTKRendererPreservesSwiftUIScrollRowWidthContract() throws {
        ...
        #expect(renderer.contains("the primitive itself should fit content"))
        ...
        #expect(renderer.contains("fillHeight: false"))
        ...
        #expect(renderer.contains("gtk_scrolled_window_set_propagate_natural_height(scrolledOp, 1)"))
        ...
        #expect(patcher.contains("gtk_scrolled_window_set_propagate_natural_height(scrolledOp, 1)"))
        ...
        #expect(renderer.contains("fillHeight: horizontalContentWantsViewportHeight"))
        #expect(renderer.contains("let scrollerWantsVerticalFill =\n            axes.contains(.vertical) || horizontalContentWantsViewportHeight"))
        #expect(renderer.contains("gtk_widget_set_vexpand(scrolled, scrollerWantsVerticalFill ? 1 : 0)"))
        #expect(renderer.contains("gtk_widget_set_valign(scrolled, scrollerWantsVerticalFill ? GTK_ALIGN_FILL : GTK_ALIGN_START)"))
        #expect(renderer.contains("gtkMarkVerticalFillIntent(scrolled)"))
        #expect(patcher.contains("fillHeight: horizontalContentWantsViewportHeight"))
        #expect(patcher.contains("axes.contains(.vertical) || horizontalContentWantsViewportHeight"))
        #expect(patcher.contains("gtk_widget_set_valign(scrolled, scrollerWantsVerticalFill ? GTK_ALIGN_FILL : GTK_ALIGN_START)"))
        ...
        #expect(renderer.contains("for renderedChild in gtkRenderChildren(contentBuilder(item))"))
        #expect(renderer.contains("gtkLazyHStackCrossAxisAlignment(alignment)"))
        #expect(renderer.contains("gtk_widget_get_vexpand(child) != 0 ? GTK_ALIGN_FILL : crossAxisAlignment"))
        #expect(patcher.contains("SwiftOpenUI builder-style lazy stack child flattening shape was not recognized"))
        #expect(renderer.contains("extension ContainerRelativeFrameView: GTKRenderable"))
        #expect(renderer.contains("gtkInstallContainerRelativeFrameSizing("))
        #expect(renderer.contains("let horizontalContentWantsViewportHeight ="))
        #expect(renderer.contains("gtkClearVerticalFillIntent(wrapper)\n        return opaqueFromWidget(wrapper)\n    }\n}\n\n// MARK: - ViewThatFits GTK extension"))
        #expect(renderer.contains("gtkPropagateSingleChildLayoutMarkers(from: [baseWidget], to: container)"))
        #expect(patcher.contains("SwiftOpenUI GTK container-relative sizing helper insertion shape was not recognized"))
        #expect(patcher.contains("SwiftOpenUI GTK horizontal ScrollView cross-axis fill shape was not recognized"))
        #expect(patcher.contains("SwiftOpenUI GTK Overlay layout marker insertion shape was not recognized"))
    ...
    @Test("Vendored GTK background layout follows SwiftUI content size")
    func vendoredGTKBackgroundLayoutFollowsSwiftUIContentSize() throws {
        let renderer = try packageSource("third_party/SwiftOpenUI/Sources/Backend/GTK4/Rendering/GTKRenderer.swift")

        #expect(renderer.contains("private func gtkClearLayoutMarker"))
        #expect(renderer.contains("private func gtkClearVerticalFillIntent"))
        #expect(renderer.contains("gtk_widget_set_hexpand(backgroundWidget, gtk_widget_get_hexpand(contentWidget))"))
        #expect(renderer.contains("gtk_widget_set_vexpand(backgroundWidget, gtk_widget_get_vexpand(contentWidget))"))
        #expect(renderer.contains("gtk_widget_set_halign(backgroundWidget, GTK_ALIGN_FILL)"))
        #expect(renderer.contains("gtk_widget_set_valign(backgroundWidget, GTK_ALIGN_FILL)"))
        #expect(renderer.contains("if !gtkHasVerticalFillIntent(contentWidget) {\n        gtkClearVerticalFillIntent(backgroundWidget)\n    }"))
        #expect(renderer.contains("return gtkRenderFallbackZStack([backgroundWidget, contentWidget], alignment: alignment)"))
    }
    ...
    func vendoredGTKRendererAppliesSwiftUIFormRowMetadata() throws {
        ...
        #expect(rowModifiers.contains("public protocol ListRowBackgroundProvider"))
        ...
        #expect(rowModifiers.contains("public struct ListRowBackgroundView<Content: View, Background: View>: View, ListRowBackgroundProvider"))
        ...
        #expect(renderer.contains("private func gtkListRowMinimumHeight(for view: any View) -> gint"))
        #expect(renderer.contains("return environmentMinimum\n}\n\nprivate func gtkListRowEstimatedHeight(for view: any View) -> gint"))
        #expect(renderer.contains("let minimumHeight = gtkListRowMinimumHeight(for: view)"))
        #expect(renderer.contains("return max(minimumHeight, contentMinimum)"))
        #expect(renderer.contains("let includeShortPlainText = gtkViewIsPlainTextRow(view)"))
        #expect(renderer.contains("let child = gtkWithSuppressedDescriptorLifecyclePayloads"))
        #expect(renderer.contains("gtkWithRowTextRenderContext(includeShortPlainText: includeShortPlainText)"))
        ...
        #expect(renderer.contains(".swiftopenui-list row { border-bottom: 1px solid alpha(currentColor, 0.18); padding: 0; }"))
        #expect(!renderer.contains(".swiftopenui-list row { border-bottom: 1px solid alpha(currentColor, 0.18); min-height: 56px; padding: 0; }"))
        #expect(renderer.contains("gtk_widget_set_vexpand(listBox, 1)"))
        #expect(renderer.contains("gtk_widget_set_valign(listBox, GTK_ALIGN_FILL)"))
        #expect(renderer.contains("the natural-height listbox vertically"))
        #expect(renderer.contains("gtkInstallScrollViewCrossAxisFill(on: scrolled, child: listBox, fillWidth: true, fillHeight: true)"))
        #expect(renderer.contains("let estimatedHeight = gtkListRowEstimatedHeight(for: child)"))
        #expect(renderer.contains("estimatedHeight: Double(estimatedHeight)"))
        #expect(renderer.contains("gtkMarkVerticalFillIntent(scrolled)"))
        #expect(renderer.contains(".swiftopenui-list row.separator-hidden { border-bottom: none; }"))
        #expect(renderer.contains("var background: (any View)?"))
        #expect(renderer.contains("if let backgroundProvider = value as? any ListRowBackgroundProvider"))
        #expect(renderer.contains("metadata.background = backgroundProvider.listRowBackground"))
        #expect(renderer.contains("if let color = resolvedMetadata.background as? Color"))
        #expect(renderer.contains("background-color: rgba(%d, %d, %d, %.3f);"))
        ...
        #expect(!designCompat.contains("func listRowBackground<V: View>(_ view: V?) -> some View { self }"))
        #expect(rowModifiers.contains("func listRowBackground<V: View>(_ view: V?) -> ListRowBackgroundView<Self, V>"))
    ...
    func upstreamIceCubesVisualSmokeIsFirstClassLinuxArtifact() throws {
        ...
        let fetchUpstream = try packageSource("scripts/fetch-upstream.sh")
        let appKitLowering = try packageSource("Sources/QuillSourceLowering/AppKitLowering.swift")
        let appKitLoweringCLI = try packageSource("Sources/quill-lower-appkit/main.swift")
        let appKitLoweringRunner = try packageSource("scripts/run-quill-appkit-lower.sh")
        ...
        let keyboardShortcutRegistry = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/Modifiers/KeyboardShortcutRegistry.swift")
        ...
        #expect(script.contains("QUILLUI_ICECUBES_VISUAL_TYPE_INSTANCE_READY_TIMEOUT_SECONDS"))
        #expect(script.contains("xdotool key --delay \"$TYPE_KEY_DELAY_MS\" --clearmodifiers"))
        #expect(script.contains("wait_for_add_account_selected_instance_visual"))
        ...
        #expect(script.contains("QUILLUI_ICECUBES_VISUAL_FINAL_CAPTURE_RETRY_TIMEOUT_SECONDS"))
        #expect(script.contains("should_retry_final_visual_capture()"))
        #expect(script.contains("capture_and_verify_final_visual 1"))
        ...
        #expect(mastodonFixtures.contains("\"host\": \"instances.social\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/1.0/instances/list\""))
        #expect(mastodonFixtures.contains("\"query\": \"count=1000&include_closed=false&include_dead=false&min_active_users=500\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/1.0/instances/search\""))
        #expect(mastodonFixtures.contains("\"query\": \"q=mastodon.social\""))
        #expect(mastodonFixtures.contains("Fixture instance for QuillUI IceCubes Add Account parity."))
        ...
        #expect(script.contains("sign-in-callback-persistence)"))
        #expect(script.contains("QUILLUI_WEB_AUTH_CALLBACK_FILE=$OAUTH_CALLBACK_FILE_PATH"))
        #expect(script.contains("OAUTH_CALLBACK_URL=\"${QUILLUI_ICECUBES_VISUAL_OAUTH_CALLBACK_URL:-icecubesapp://oauth?code=quill-oauth-code}\""))
        #expect(script.contains("OAUTH_TOKEN_EXCHANGE_LOG=\"[QuillURLSessionFixtures] direct POST https://mastodon.social/oauth/token\""))
        #expect(script.contains("OAUTH_VERIFY_CREDENTIALS_LOG=\"[QuillURLSessionFixtures] direct GET https://mastodon.social/api/v1/accounts/verify_credentials\""))
        #expect(script.contains("wait_for_oauth_callback_completion"))
        #expect(script.contains("verify_persisted_oauth_account"))
        #expect(script.contains("relaunch_icecubes_with_persisted_oauth_account"))
        #expect(script.contains("start_icecubes_app append oauth-relaunch"))
        #expect(script.contains("persisted OAuth authenticated relaunch"))
        #expect(script.contains("\"accessToken\") != \"quill-oauth-access-token\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/apps\""))
        #expect(mastodonFixtures.contains("\"path\": \"/oauth/token\""))
        #expect(mastodonFixtures.contains("\"client_id\": \"quill-oauth-client\""))
        #expect(mastodonFixtures.contains("\"code\": \"quill-oauth-code\""))
        #expect(mastodonFixtures.contains("\"access_token\": \"quill-oauth-access-token\""))
        ...
        #expect(seedMain.contains("store.set(true, forKey: \"is_previously_set\")"))
        #expect(seedMain.contains("store.set(\"light\", forKey: \"selectedScheme\")"))
        #expect(seedMain.contains("store.set(\"Ice Cube - Light\", forKey: \"selectedSet\")"))
        #expect(seedMain.contains("store.set(false, forKey: \"followSystemColorSchme\")"))
        #expect(seedMain.contains("store.set(0xBB3BE2, forKey: \"tint\")"))
        #expect(seedMain.contains("store.set(0xFFFFFF, forKey: \"primaryBackground\")"))
        #expect(seedMain.contains("store.set(0xF0F1F2, forKey: \"secondaryBackground\")"))
        #expect(seedMain.contains("store.set(0x000000, forKey: \"label\")"))
        ...
        #expect(script.contains("AUTH_COMPOSER_SEND_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_SEND_X:-672}\""))
        #expect(script.contains("AUTH_COMPOSER_SEND_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_SEND_Y:-60}\""))
        #expect(script.contains("AUTH_COMPOSER_SEND_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_SEND_POINTS:-$AUTH_COMPOSER_SEND_X,$AUTH_COMPOSER_SEND_Y 606,93 755,111}\""))
        #expect(script.contains("AUTH_COMPOSER_SEND_CLICK_RETRY_POLLS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_SEND_CLICK_RETRY_POLLS:-20}\""))
        #expect(script.contains("click_authenticated_composer_send_button \"$((previous_submit_count + 1))\""))
        #expect(script.contains("wait_for_authenticated_composer_submit_after_click()"))
        #expect(script.contains("AUTH_COMPOSER_TYPE_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_TYPE_Y:-180}\""))
        #expect(script.contains("AUTH_COMPOSER_TYPE_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_TYPE_POINTS:-$AUTH_COMPOSER_TYPE_X,$AUTH_COMPOSER_TYPE_Y 320,150 320,285}\""))
        #expect(script.contains("QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_TYPED_CHANGE_MIN_PIXELS"))
        #expect(script.contains("composer_typed_change_pixels()"))
        #expect(script.contains("IceCubes authenticated composer did not visibly accept typed text at any configured point."))
        #expect(script.contains("QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_TYPE_FOCUS_SETTLE_SECONDS"))
        #expect(script.contains("sleep \"$AUTH_COMPOSER_TYPE_FOCUS_SETTLE_SECONDS\""))
        #expect(script.contains("QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSE_CLICK_RETRIES"))
        #expect(script.contains("open_authenticated_composer_surface()"))
        #expect(script.contains("IceCubes authenticated compose button did not open the composer after $attempt attempts."))
        ...
        #expect(script.contains("open_authenticated_composer_surface"))
        #expect(script.contains("local verify_product=\"${1:-icecubes-linux-authenticated-composer}\""))
        #expect(script.contains("\"$ROOT_DIR/scripts/verify-backend-screenshot.py\" \"$probe_path\" \"$verify_product\" 2>&1"))
        ...
        #expect(script.contains("AUTH_EXPLORE_SUGGESTED_USERS_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_EXPLORE_SUGGESTED_USERS_Y:-128}\""))
        #expect(script.contains("AUTH_EXPLORE_TAGS_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_EXPLORE_TAGS_X:-682}\""))
        #expect(script.contains("AUTH_EXPLORE_TAGS_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_EXPLORE_TAGS_Y:-128}\""))
        #expect(script.contains("AUTH_EXPLORE_SUGGESTED_RELATIONSHIPS_ENDPOINT=\"/api/v1/accounts/relationships?id%5B%5D=suggested-account-1&id%5B%5D=suggested-account-2\""))
        ...
        #expect(script.contains("AUTH_PROFILE_STATUSES_ENDPOINT=\"/api/v1/accounts/quill-account/statuses\""))
        #expect(script.contains("AUTH_PROFILE_CLICK_RETRIES=\"${QUILLUI_ICECUBES_VISUAL_AUTH_PROFILE_CLICK_RETRIES:-5}\""))
        #expect(script.contains("AUTH_PROFILE_CLICK_RETRY_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_PROFILE_CLICK_RETRY_SECONDS:-0.75}\""))
        ...
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_Y:-325}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_ROUTE_SETTLE_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_ROUTE_SETTLE_SECONDS:-0.6}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_CONTROLS_SCROLL_KEYS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_CONTROLS_SCROLL_KEYS:-Page_Down}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_CONTROLS_KEY_DELAY_MS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_CONTROLS_KEY_DELAY_MS:-80}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_SCALE_START_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_SCALE_START_X:-}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_SCALE_END_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_SCALE_END_X:-}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_SCALE_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_SCALE_Y:-}\""))
        ...
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_SYSTEM_COLOR_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_SYSTEM_COLOR_Y:-278}\""))
        ...
        #expect(script.contains("previous_suggested_relationships_count=\"$(count_app_log_occurrences \"$AUTH_EXPLORE_SUGGESTED_RELATIONSHIPS_ENDPOINT\")\""))
        #expect(script.contains("authenticated Explore suggested account relationships"))
        ...
        #expect(script.contains("wait_for_authenticated_api_activity \"$AUTH_EXPLORE_SUGGESTED_RELATIONSHIPS_ENDPOINT\" \"authenticated Explore Suggested Users relationships\""))
        ...
        #expect(script.contains("click_authenticated_profile_sidebar_row()"))
        #expect(script.contains("previous_profile_statuses_count=\"$(count_app_log_occurrences \"$AUTH_PROFILE_STATUSES_ENDPOINT\")\""))
        #expect(script.contains("click_authenticated_profile_sidebar_row \"$expected_profile_statuses_count\""))
        #expect(script.contains("Profile sidebar click did not trigger $AUTH_PROFILE_STATUSES_ENDPOINT"))
        ...
        #expect(script.contains("seeded-authenticated-account-lifecycle)"))
        #expect(script.contains("open_authenticated_current_account_settings()"))
        #expect(script.contains("remove_authenticated_current_account()"))
        #expect(script.contains("icecubes-linux-authenticated-account-settings"))
        #expect(script.contains("AUTH_ACCOUNT_LOGOUT_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_ACCOUNT_LOGOUT_Y:-487}\""))
        #expect(script.contains("AUTH_ACCOUNT_LOGOUT_SCROLL_CLICKS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_ACCOUNT_LOGOUT_SCROLL_CLICKS:-0}\""))
        ...
        #expect(script.contains("icecubes-linux-authenticated-settings-display-font-scale-control"))
        #expect(script.contains("read -r detected_start_x detected_end_x detected_y"))
        ...
        #expect(script.contains("read -r -a settings_display_scroll_keys <<<\"$AUTH_SETTINGS_DISPLAY_CONTROLS_SCROLL_KEYS\""))
        #expect(script.contains("--clearmodifiers \"${settings_display_scroll_keys[@]}\""))
        ...
        #expect(verifier.contains("def icecubes_suggestion_media_pixel"))
        #expect(verifier.contains("loaded_thumbnail = max(rgb) <= 24"))
        #expect(verifier.contains("if icecubes_suggestion_media_pixel(image.rgb(x, y))"))
        ...
        #expect(verifier.contains("top + 420"))
        #expect(verifier.contains("top + 210"))
        #expect(verifier.contains("top + 285"))
        ...
        #expect(verifier.contains("\"icecubes-linux-authenticated-account-settings\""))
        #expect(verifier.contains("def validate_icecubes_linux_authenticated_account_settings"))
        #expect(verifier.contains("IceCubes authenticated Account Settings action rows are incomplete"))
        ...
        #expect(verifier.contains("def locate_icecubes_linux_authenticated_settings_display_font_scale_control"))
        #expect(verifier.contains("min_width=80"))
        ...
        #expect(verifier.contains("min_display_row_segments: int = 7"))
        #expect(verifier.contains("display_row_segments >= min_display_row_segments"))
        #expect(verifier.contains("min_display_row_segments=6"))
        ...
        #expect(verifier.contains("toggle_accent_pixels >= 72"))
        #expect(verifier.contains("active_color_row_text_pixels <= 120"))
        #expect(verifier.contains("IceCubes authenticated Settings Display color rows did not become disabled after system-color toggle"))
        ...
        #expect(gtkNavigation.contains("private var gtkNavigationPersistedRoutesBySignature"))
        #expect(gtkNavigation.contains("private func gtkNavigationRestoreSignature"))
        #expect(gtkNavigation.contains("var restoreSignature: String?"))
        #expect(gtkNavigation.contains("gtkNavigationPersistedRoutes(for: stateNamespace, restoreSignature: restoreSignature)"))
        #expect(gtkNavigation.contains("context.restoreSignature = gtkNavigationRestoreSignature(title: title, rootWidget: rootWidget)"))
        ...
        #expect(gtkNavigation.contains("struct GTKNavigationToolbarSnapshot"))
        #expect(gtkNavigation.contains("func replaceCurrentToolbar(with snapshot: GTKNavigationToolbarSnapshot)"))
        #expect(gtkNavigation.contains("func gtkNavigationToolbarSnapshot<V: View>(from view: V) -> GTKNavigationToolbarSnapshot?"))
        #expect(gtkNavigation.contains("gtkExtractToolbarItemsTyped(from: view.body, depth: depth + 1)"))
        #expect(gtkNavigation.contains("gtkExtractToolbarConfigurationTyped(from: view.body, depth: depth + 1)"))
        #expect(gtkShim.contains("gtk_swift_root_point_pick_toggle_control"))
        #expect(gtkShim.contains("GTK_IS_CHECK_BUTTON(picked) || GTK_IS_SWITCH(picked)"))
        #expect(gtkShim.contains("gtk_swift_widget_is_check_button"))
        #expect(gtkShim.contains("gtk_swift_widget_is_switch"))
        #expect(gtkRenderer.contains("private final class GTKToggleActionBox"))
        #expect(gtkRenderer.contains("private let gtkToggleActionDataKey"))
        #expect(gtkRenderer.contains("private let gtkToggleGlobalDispatcherDataKey"))
        #expect(gtkRenderer.contains("gtkAttachToggleActionData(to: check, box: actionBox)"))
        #expect(gtkRenderer.contains("gtkAttachToggleActionData(to: sw, box: actionBox)"))
        #expect(gtkRenderer.contains("gtkAttachToggleActionData(to: paintedWidget, box: actionBox)"))
        #expect(gtkRenderer.contains("gtkAttachToggleActionData(to: hbox, box: actionBox)"))
        #expect(gtkRenderer.contains("gtkInstallToggleRootDispatcherWhenMapped(check)"))
        #expect(gtkRenderer.contains("gtkInstallToggleRootDispatcherWhenMapped(sw)"))
        #expect(gtkRenderer.contains("gtkInstallToggleRootDispatcherWhenMapped(paintedWidget)"))
        #expect(gtkRenderer.contains("gtkInstallToggleRootDispatcherWhenMapped(hbox)"))
        #expect(gtkRenderer.contains("private func gtkInstallGlobalToggleRootDispatcher"))
        #expect(gtkRenderer.contains("source: \"toggle-root-dispatch@\\(Int(rootX)),\\(Int(rootY))\""))
        #expect(gtkRenderer.contains("private func gtkToggleControlAtRootPoint"))
        #expect(gtkRenderer.contains("private func gtkVisualToggleControlAtRootPoint"))
        #expect(gtkRenderer.contains("private func gtkToggleActionWidgetAtRootPoint"))
        #expect(gtkRenderer.contains("gtkToggleActionBox(from: control)"))
        #expect(gtkRenderer.contains("box.activate(next, source: \"\\(source) check\", applyVisualState: true)"))
        #expect(gtkRenderer.contains("box.activate(next, source: \"\\(source) switch\", applyVisualState: true)"))
        #expect(gtkRenderer.contains("box.toggle(source: \"\\(source) action-widget\")"))
        #expect(gtkRenderer.contains("toggle binding set"))
        #expect(gtkRenderer.contains("toggle visual fallback"))
        #expect(gtkRenderer.contains("toggle action widget activated"))
        #expect(gtkRenderer.contains("private func gtkActivateToggleControlAtRootPoint"))
        #expect(gtkRenderer.contains("gtk_swift_root_point_pick_toggle_control(root, x, y)"))
        #expect(gtkRenderer.contains("gtk_check_button_set_active(check, next ? 1 : 0)"))
        #expect(gtkRenderer.contains("gtk_swift_switch_set_active(control, next ? 1 : 0)"))
        #expect(gtkRenderer.contains("toggle control check activated"))
        #expect(gtkRenderer.contains("gtkActivateToggleControlAtRootPoint(\n                root: root,\n                x: rootX,"))
        #expect(gtkRenderer.contains("gtkActivateToggleControlAtWidgetPoint("))
        #expect(gtkViewHost.contains("var navigationToolbarSnapshot: (() -> GTKNavigationToolbarSnapshot?)?"))
        #expect(gtkViewHost.contains("func refreshNavigationToolbarFromBodyIfNeeded()"))
        #expect(gtkViewHost.contains("navigationContext.replaceCurrentToolbar(with: snapshot)"))
        #expect(gtkViewHost.contains("refreshNavigationToolbarFromBodyIfNeeded()"))
        #expect(gtkRenderer.contains("host.navigationToolbarSnapshot = {"))
        #expect(gtkRenderer.contains("gtkNavigationToolbarSnapshot(from: view.body)"))
        #expect(gtkRenderer.contains("host.refreshNavigationToolbarFromBodyIfNeeded()"))
        #expect(gtkNavigation.contains("gtk_box_append(boxPointer(container), headerBar)"))
        #expect(gtkNavigation.contains("Keep the navigation context and installed sheet chrome pointed at"))
        #expect(!gtkNavigation.contains("gtk_box_append(boxPointer(container), inlineBar)"))
        ...
        #expect(verifier.contains("validate_icecubes_linux_authenticated_composer_submitted(image)"))
        #expect(verifier.contains("left + int(app_width * 0.55)"))
        #expect(verifier.contains("right - 320"))
        #expect(verifier.contains("top + 58"))
        #expect(verifier.contains("min(right - 80, left + int(app_width * 0.88))"))
        #expect(verifier.contains("min(bottom, top + 132)"))
        #expect(verifier.contains("composer_send_button_pixels >= 200"))
        #expect(verifier.contains("typed_leading_editor_pixels"))
        #expect(verifier.contains("typed_text_pixels = max(typed_body_pixels, typed_leading_editor_pixels)"))
        #expect(verifier.contains("typed_text_pixels >= 200"))
        #expect(verifier.contains("composer_media_viewer_pixels <= 30_000"))
        #expect(verifier.contains("allow_quoted_media: bool = False"))
        #expect(verifier.contains("composer_media_viewer_pixels <= 230_000"))
        #expect(verifier.contains("\"icecubes-linux-authenticated-reply-composer\""))
        #expect(verifier.contains("validate_icecubes_linux_authenticated_composer(image, allow_quoted_media=True)"))
        #expect(verifier.contains("IceCubes authenticated composer capture appears to show a media viewer instead of the editor"))
        ...
        #expect(verifier.contains("first_notification_divider = best_horizontal_segment("))
        #expect(verifier.contains("IceCubes authenticated Notifications first row was not compact"))
        #expect(verifier.contains("left + 305"))
        #expect(verifier.contains("first_notification_divider_y - 4"))
        #expect(verifier.contains("first_notification_body_pixels"))
        #expect(verifier.contains("second_notification_action_pixels"))
        #expect(verifier.contains("400 <= first_notification_body_pixels <= 2_400"))
        #expect(verifier.contains("second_notification_action_pixels >= 250"))
        ...
        #expect(verifier.contains("profile_options_button_pixels"))
        #expect(verifier.contains("IceCubes authenticated Profile options toolbar button was not detected"))
        ...
        #expect(script.contains("AUTH_STATUS_DETAIL_REPLY_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_REPLY_X:-319}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_REPLY_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_REPLY_Y:-436}\""))
        #expect(script.contains("local verify_product=\"${1:-icecubes-linux-authenticated-composer}\""))
        #expect(script.contains("verify-backend-screenshot.py\" \"$probe_path\" \"$verify_product\""))
        #expect(script.contains("seeded-authenticated-status-detail-reply)"))
        #expect(script.contains("VERIFY_PRODUCT=\"icecubes-linux-authenticated-reply-composer\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOST_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_X:-372}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOST_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_Y:-418}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOST_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_POINTS:-$AUTH_STATUS_DETAIL_BOOST_X,$AUTH_STATUS_DETAIL_BOOST_Y $AUTH_STATUS_DETAIL_BOOST_X,428"))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOST_MENU_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_MENU_X:-425}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOST_MENU_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_MENU_Y:-530}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOST_MENU_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_MENU_POINTS:-$AUTH_STATUS_DETAIL_BOOST_MENU_X,$AUTH_STATUS_DETAIL_BOOST_MENU_Y 400,$AUTH_STATUS_DETAIL_BOOST_MENU_Y"))
        #expect(script.contains("Invalid QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_POINTS entry"))
        #expect(script.contains("Invalid QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOST_MENU_POINTS entry"))
        #expect(script.contains("AUTH_STATUS_DETAIL_QUOTE_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_QUOTE_POINTS:-$AUTH_STATUS_DETAIL_BOOST_X,$AUTH_STATUS_DETAIL_BOOST_Y"))
        #expect(script.contains("AUTH_STATUS_DETAIL_QUOTE_MENU_Y=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_QUOTE_MENU_Y:-568}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_QUOTE_MENU_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_QUOTE_MENU_POINTS:-$AUTH_STATUS_DETAIL_QUOTE_MENU_X,$AUTH_STATUS_DETAIL_QUOTE_MENU_Y"))
        #expect(script.contains("wait_for_authenticated_status_detail_menu_open"))
        #expect(script.contains("AUTH_STATUS_DETAIL_MENU_OPEN_LOG=\"[QuillUI GTK] menu overlay opened\""))
        #expect(script.contains("Invalid QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_QUOTE_POINTS entry"))
        #expect(script.contains("Invalid QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_QUOTE_MENU_POINTS entry"))
        #expect(script.contains("click_authenticated_status_detail_boost_action"))
        #expect(script.contains("click_authenticated_status_detail_quote_action"))
        #expect(script.contains("click_authenticated_status_detail_reply_action"))
        #expect(script.contains("xdotool key --clearmodifiers Escape"))
        #expect(script.contains("AUTH_STATUS_DETAIL_FAVORITE_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_FAVORITE_POINTS:-$AUTH_STATUS_DETAIL_FAVORITE_X,$AUTH_STATUS_DETAIL_FAVORITE_Y 413,$AUTH_STATUS_DETAIL_FAVORITE_Y"))
        #expect(script.contains("for point in $AUTH_STATUS_DETAIL_FAVORITE_POINTS"))
        #expect(script.contains("Invalid QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_FAVORITE_POINTS entry"))
        #expect(script.contains("click_authenticated_status_detail_favorite_action"))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOKMARK_X=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOKMARK_X:-483}\""))
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOKMARK_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOKMARK_POINTS:-$AUTH_STATUS_DETAIL_BOOKMARK_X,$AUTH_STATUS_DETAIL_BOOKMARK_Y 471,$AUTH_STATUS_DETAIL_BOOKMARK_Y"))
        #expect(!script.contains("AUTH_STATUS_DETAIL_BOOKMARK_POINTS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOKMARK_POINTS:-$AUTH_STATUS_DETAIL_SECONDARY_BOOKMARK_X,432"))
        #expect(script.contains("for point in $AUTH_STATUS_DETAIL_BOOKMARK_POINTS"))
        #expect(script.contains("Invalid QUILLUI_ICECUBES_VISUAL_AUTH_STATUS_DETAIL_BOOKMARK_POINTS entry"))
        #expect(script.contains("click_authenticated_status_detail_bookmark_action"))
        #expect(script.contains("count_authenticated_status_detail_bookmark_actions()"))
        #expect(script.contains("wait_for_authenticated_status_detail_bookmark_action()"))
        ...
        #expect(verifier.contains("def validate_icecubes_linux_authenticated_status_detail_bookmark"))
        ...
        #expect(verifier.contains("detail_has_media = detail_media_pixels >= 80_000"))
        #expect(!verifier.contains("detail_has_summary = detail_summary_pixels >= 350"))
        #expect(verifier.contains("IceCubes authenticated Status detail media attachment was not detected"))
        #expect(verifier.contains("detail_action_pixels"))
        #expect(verifier.contains("detail_top_action_pixels"))
        #expect(verifier.contains("detail_media_action_pixels"))
        #expect(verifier.contains("detail_centered_media_action_count_pixels"))
        #expect(verifier.contains("detail_action_count_pixels"))
        #expect(verifier.contains("boost_accent_pixels"))
        #expect(verifier.contains("boost_media_accent_pixels"))
        #expect(verifier.contains("boost_media_summary_pixels"))
        #expect(verifier.contains("boost_top_action_guard_pixels"))
        #expect(verifier.contains("favorite_accent_pixels"))
        #expect(verifier.contains("favorite_media_accent_pixels"))
        #expect(verifier.contains("favorite_media_summary_pixels"))
        #expect(verifier.contains("favorite_top_action_guard_pixels"))
        #expect(verifier.contains("bookmark_media_accent_pixels"))
        #expect(verifier.contains("bookmark_top_action_guard_pixels"))
        #expect(verifier.contains("left + 555"))
        #expect(verifier.contains("left + 620"))
        ...
        #expect(gtkNavigation.contains("struct GTKNavigationToolbarSignature: Equatable"))
        #expect(gtkNavigation.contains("var toolbarSignature: GTKNavigationToolbarSignature?"))
        #expect(gtkNavigation.contains("guard current.toolbarWidgets.isEmpty else { return }"))
        #expect(gtkNavigation.contains("guard current.toolbarSignature != signature else { return }"))
        #expect(gtkNavigation.contains("entry.toolbarSignature = signature"))
        #expect(gtkNavigation.contains("for itemWidget in gtkRenderToolbarItemWidgets(item)"))
        #expect(gtkNavigation.contains("let renderedTypes = item.renderedViews"))
        ...
        #expect(gtkDescriptorTree.contains("public func gtkCaptureRenderLifecyclePayloads"))
        #expect(gtkDescriptorTree.contains("func gtkWithOwnedDescriptorLifecyclePayloads"))
        #expect(gtkViewHost.contains("var lastRenderTaskPayloads: [GTK4TaskPayload] = []"))
        #expect(gtkViewHost.contains("buildBodyCapturingRenderLifecyclePayloads()"))
        #expect(gtkViewHost.components(separatedBy: "gtkWithOwnedDescriptorLifecyclePayloads").count >= 3)
        #expect(gtkViewHost.contains("renderCapturedTaskPayloads(fallback: described.taskPayloads)"))
        #expect(gtkRenderer.contains("gtkCollectTaskPayload(\n                GTK4TaskPayload("))
        #expect(gtkRenderer.contains("gtkCollectOnAppearPayload(GTK4OnAppearPayload(action: boundAction))"))
        #expect(gtkRenderer.contains("let child = gtkWithSuppressedDescriptorLifecyclePayloads"))
        #expect(patcher.contains("SwiftOpenUI TaskView render lifecycle payload shape was not recognized"))
        #expect(patcher.contains("gtkCaptureRenderLifecyclePayloads"))
        #expect(patcher.contains("SwiftOpenUI GTKViewHost lifecycle-owned render shape was not recognized"))
        #expect(patcher.contains("renderCapturedTaskPayloads(fallback: described.taskPayloads)"))
        #expect(patcher.contains("SwiftOpenUI List row minimum-height shape was not recognized"))
        #expect(patcher.contains("private func gtkListRowEstimatedHeight(for view: any View) -> gint"))
        #expect(patcher.contains("let estimatedHeight = gtkListRowEstimatedHeight(for: child)"))
        #expect(patcher.contains("estimatedHeight: Double(estimatedHeight)"))
        ...
        #expect(gtkDescriptorTree.contains("kind = .statefulLifecycleScope"))
        #expect(gtkDescriptorTree.contains("child = gtkWithSuppressedDescriptorLifecyclePayloads {"))
        #expect(gtkDescriptorTree.contains("children: [child]"))
        #expect(gtkDescriptorTree.contains("MainActor.assumeIsolated { gtkDescribeAnyView(view.body) }"))
        #expect(gtkViewHost.contains("gtkWithForcedStateIdentityNamespace(stateIdentityNamespace)"))
        #expect(patcher.contains("SwiftOpenUI stateful descriptor lifecycle ownership shape was not recognized"))
        #expect(patcher.contains("SwiftOpenUI GTKViewHost lifecycle-owned describe shape was not recognized"))
        ...
        #expect(gtkRenderer.contains("gtk_widget_set_hexpand(box, 1)"))
        #expect(gtkRenderer.contains("gtk_widget_set_vexpand(box, 1)"))
        #expect(gtkRenderer.contains("gtk_widget_set_valign(box, GTK_ALIGN_FILL)"))
        #expect(gtkRenderer.contains("gtkMarkVerticalFillIntent(box)"))
        ...
        #expect(gtkRenderer.contains("gtk_widget_set_hexpand(contentWidget, 1)"))
        #expect(gtkRenderer.contains("gtk_widget_set_valign(contentWidget, GTK_ALIGN_FILL)"))
        ...
        #expect(gtkRenderer.contains("gtkScheduleTextBindingUpdate(box.updateSource, value: newValue)"))
        ...
        #expect(patcher.contains("gtk_widget_set_valign(contentWidget, GTK_ALIGN_FILL)"))
        #expect(patcher.contains("gtkMarkVerticalFillIntent(box)"))
        ...
        #expect(workflow.contains("GTK IceCubes media layout tests"))
        #expect(workflow.contains("--filter GTKCustomLayoutTests"))
        ...
        #expect(workflow.contains("Upstream IceCubes GTK OAuth callback and persisted relaunch smoke"))
        #expect(workflow.contains("QUILLUI_ICECUBES_VISUAL_INTERACTION=sign-in-callback-persistence"))
        #expect(workflow.contains(".qa/icecubes-linux-oauth-callback-persisted.png"))
        ...
        #expect(parityLog.contains("sign-in-callback-persistence"))
        #expect(parityLog.contains("authenticated process relaunch"))
        #expect(parityLog.contains("no IceCubes source is patched for this path"))
        ...
        #expect(fetchUpstream.contains("run-quill-appkit-lower.sh\" --application-delegates-only \"$appdir"))
        #expect(appKitLowering.contains("public struct ApplicationDelegateAdaptorLowering"))
        #expect(appKitLowering.contains("ApplicationDelegateConstructionRewriter().rewrite(tree).description"))
        #expect(appKitLoweringCLI.contains("--application-delegates-only"))
        #expect(appKitLoweringCLI.contains("ApplicationDelegateAdaptorLowering().lowerInPlace"))
        #expect(appKitLoweringRunner.contains("quill-lower-appkit"))
        #expect(appKitLoweringRunner.contains("\"$@\""))
        ...
        #expect(mastodonFixtures.contains("\"query\": \"max_id=1001&grouped_types%5B%5D=favourite&grouped_types%5B%5D=follow&grouped_types%5B%5D=reblog&expand_accounts=full\""))
        ...
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/statuses/1003/favourited_by\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/statuses/1003/reblogged_by\""))
        ...
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/statuses/1003/bookmark\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/statuses/1001/bookmark\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/statuses/999/bookmark\""))
        ...
        #expect(script.contains("AUTH_REFRESH_KEY_RETRIES"))
        #expect(script.contains("AUTH_REFRESH_KEY_RETRY_SECONDS"))
        #expect(script.contains("trigger_authenticated_refresh_shortcut()"))
        ...
        #expect(script.contains("expected_refresh_trigger_count=\"$((previous_refresh_trigger_count + 1))\""))
        #expect(script.contains("for attempt in $(seq 1 \"$AUTH_REFRESH_KEY_RETRIES\"); do"))
        #expect(script.contains("wait_for_app_log_activity \"[QuillUI GTK Refreshable] trigger source=keyboard\" \"$label shortcut\" \"$expected_refresh_trigger_count\""))
        ...
        #expect(gtkRenderer.contains("gtk_swift_is_widget(widget) != 0 && gtk_widget_get_mapped(widget) != 0"))
        #expect(keyboardShortcutRegistry.contains("isEnabled: @escaping () -> Bool = { true }"))
        #expect(keyboardShortcutRegistry.contains("let match = candidates.last(where: { $0.isEnabled() })"))
        ...
        #expect(gtkRenderer.contains("extension AnyView: GTKRenderable, GTKDescribable"))
        #expect(gtkRenderer.contains("children: [gtkDescribeAnyView(wrapped)]"))
        ...
        #expect(gtkRenderer.contains("let bottomPrefetchMargin = max(8.0, min(320.0, viewportHeight * 0.5))"))
        #expect(gtkRenderer.contains("Double(rowY) < viewportHeight + bottomPrefetchMargin"))
        #expect(gtkRenderer.contains("deinit {\n        cancelActiveTasks()\n    }"))
        #expect(gtkRenderer.contains("if let activeTask = activeTasksByIdentity[identity],\n           activeTask.lifecycleID == payload.lifecycleID"))
        #expect(!gtkRenderer.contains("activeTasksByIdentity[identity]?.lifecycleID == payload.lifecycleID"))
        #expect(!gtkRenderer.contains("gtkListDebugLog(\"row lifecycle hidden source=\\(source)\")\n            isVisible = false\n            cancelActiveTasks()"))
        ...
        #expect(script.contains("AUTH_STATUS_DETAIL_BOOKMARK_POST_LOG=\"POST https://mastodon.social/api/v1/statuses/1003/bookmark\""))
        #expect(!script.contains("AUTH_STATUS_DETAIL_SECONDARY_BOOKMARK_POST_LOG"))
        #expect(!script.contains("AUTH_STATUS_DETAIL_PAGINATED_BOOKMARK_POST_LOG"))
        #expect(script.contains("count_app_log_occurrences \"$AUTH_STATUS_DETAIL_BOOKMARK_POST_LOG\""))
        #expect(!script.contains("primary_count + secondary_count"))
        ...
        #expect(script.contains("authenticated status detail favorited-by fetch"))
        #expect(script.contains("authenticated status detail reblogged-by fetch"))
        ...
        #expect(gtkRenderer.contains("if let multi = view as? any TransparentMultiChildView"))
        #expect(gtkRenderer.contains("func gtkTestActivatePrimaryTapAction<V: View>"))
        #expect(patcher.contains("SwiftOpenUI transparent primary-action traversal shape was not recognized"))
        ...
        #expect(gtkRenderer.contains("private typealias GTKVisualTapActionCandidate"))
        #expect(gtkRenderer.contains("private func gtkPreferredVisualTapActionCandidate"))
        #expect(gtkRenderer.contains("private func gtkVisualTapActionCandidateAtRootPoint"))
        #expect(gtkRenderer.contains("private func gtkPreferredTapGestureActionBoxAtRootPoint"))
        #expect(gtkRenderer.contains("let pickedBox = gtkPickedTapActionWidgetAtRootPoint"))
        #expect(gtkRenderer.contains("let visualBox = gtkVisualTapActionCandidateAtRootPoint"))
        #expect(gtkRenderer.contains("return pickedBox ?? visualBox"))
    ...
    func gtkPickerVisualHitCompatibilityStaysReproducible() throws {
        ...
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_PICKER_OPEN_RETRIES=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_PICKER_OPEN_RETRIES:-3}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_RETRIES=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_RETRIES:-3}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_RETRY_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_RETRY_SECONDS:-0.75}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_FOCUS_SETTLE_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_FOCUS_SETTLE_SECONDS:-0.5}\""))
        #expect(script.contains("AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_POINTER_SETTLE_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_SETTINGS_DISPLAY_FONT_PICKER_SELECT_POINTER_SETTLE_SECONDS:-0.5}\""))
        #expect(script.contains("if authenticated_route_visual_is_ready \"icecubes-linux-authenticated-settings-display-font-picker\"; then"))
        ...
        #expect(script.contains("authenticated_route_visual_is_ready \"icecubes-linux-authenticated-settings-display-font-picker-selected\""))
        #expect(script.contains("authenticated_route_visual_is_ready \"icecubes-linux-authenticated-settings-display-font-picker\""))
    ...
    func uiKitRepresentablesAreOpaqueToGTKMetadataExtraction() throws {
        ...
        let gtkDescriptorTree = try packageSource(
            "third_party/SwiftOpenUI/Sources/Backend/GTK4/Rendering/GTK4DescriptorTree.swift"
        )
        ...
        #expect(gtkDescriptorTree.contains("if view is any _ViewMetadataExtractionBoundary"))
    ...
    func iceCubesComposerMediaAttachmentCompatibilityIsReproducible() throws {
        ...
        let mastodonFixtures = try packageSource("Tests/Fixtures/IceCubes/mastodon-fixtures.json")
        let designSystemCompat = try packageSource("Sources/QuillSwiftUICompatibility/DesignSystemSurfaceCompat.swift")
        let onChangeModifier = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/Modifiers/OnChangeModifier.swift")
        let frameModifier = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/Modifiers/FrameModifier.swift")
        let asyncImage = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/Views/AsyncImage.swift")
        let environment = try packageSource("third_party/SwiftOpenUI/Sources/SwiftOpenUI/Environment/Environment.swift")
        let gtkRenderer = try packageSource("third_party/SwiftOpenUI/Sources/Backend/GTK4/Rendering/GTKRenderer.swift")
        let qtRenderer = try packageSource("Sources/BackendQt/QtRenderer.swift")
        let winRenderer = try packageSource("third_party/SwiftOpenUI/Sources/Backend/Win32/Rendering/WinRenderer.swift")
        let fileImporter = try packageSource("Sources/QuillSwiftUICompatibility/QuillFileImporter.swift")
        let photosUI = try packageSource("Sources/PhotosUI/PhotosUI.swift")

        #expect(script.contains("seeded-authenticated-composer-media-attachment"))
        #expect(script.contains("Tests/Fixtures/IceCubes/composer-upload.png"))
        #expect(script.contains("QUILLUI_FILE_IMPORTER_SELECTION=$AUTH_COMPOSER_MEDIA_SELECTION_FILE"))
        #expect(script.contains("open_authenticated_composer_media_panel"))
        #expect(script.contains("AUTH_COMPOSER_CHILD_CLICK_SETTLE_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_CHILD_CLICK_SETTLE_SECONDS:-0.25}\""))
        #expect(script.contains("AUTH_COMPOSER_CHILD_CLICK_RETRIES=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_CHILD_CLICK_RETRIES:-3}\""))
        #expect(script.contains("AUTH_COMPOSER_CHILD_CLICK_RETRY_SECONDS=\"${QUILLUI_ICECUBES_VISUAL_AUTH_COMPOSER_CHILD_CLICK_RETRY_SECONDS:-0.75}\""))
        #expect(script.contains("sleep \"$AUTH_COMPOSER_CHILD_CLICK_SETTLE_SECONDS\""))
        #expect(script.contains("click_until_authenticated_compose_surface()"))
        #expect(script.contains("wait_for_authenticated_compose_surface \"$expected_product\" 0"))
        #expect(script.contains("wait_for_authenticated_compose_surface \"$current_product\" 0"))
        #expect(script.contains("GTK can consume a synthetic press while focusing a newly mounted child."))
        #expect(script.contains("Recheck after the grace period immediately before another press."))
        #expect(script.contains("media-panel transition unexpectedly activated the Library action"))
        #expect(script.contains("\"icecubes-linux-authenticated-composer-media-panel-closed\""))
        #expect(script.contains("\"icecubes-linux-authenticated-composer-media-panel\""))
        #expect(script.contains("\"icecubes-linux-authenticated-composer-media-alt-editor\""))
        #expect(script.contains("\"icecubes-linux-authenticated-composer-media-deleted\""))
        #expect(script.contains("attach_authenticated_composer_image"))
        #expect(script.contains("seeded-authenticated-composer-media-alt-edit"))
        #expect(script.contains("edit_authenticated_composer_media_alt_text"))
        #expect(script.contains("AUTH_COMPOSER_MEDIA_ALT_UPDATE_LOG=\"[QuillURLSessionFixtures] direct PUT https://mastodon.social/api/v1/media/quill-composer-upload-1\""))
        #expect(script.contains("seeded-authenticated-composer-media-delete"))
        #expect(script.contains("delete_authenticated_composer_image"))
        #expect(verifier.contains("def validate_icecubes_linux_authenticated_composer_media_panel"))
        #expect(verifier.contains("def validate_icecubes_linux_authenticated_composer_media_panel_closed"))
        #expect(verifier.contains("thumbnail_pixels <= 5_000"))
        #expect(verifier.contains("\"icecubes-linux-authenticated-composer-media-attachment\""))
        #expect(verifier.contains("\"icecubes-linux-authenticated-composer-media-alt-saved\""))
        #expect(verifier.contains("description_text_pixels >= 100"))
        #expect(verifier.contains("generate_button_pixels <= 120"))
        #expect(verifier.contains("edge_media_pixels <= 250"))
        #expect(verifier.contains("\"icecubes-linux-authenticated-composer-media-deleted\""))
        #expect(verifier.contains("8_000 <= composer_media_viewer_pixels <= 230_000"))
        #expect(workflow.contains("seeded-authenticated-composer-media-attachment"))
        #expect(workflow.contains("seeded-authenticated-composer-media-alt-edit"))
        #expect(workflow.contains("seeded-authenticated-composer-media-delete"))
        #expect(packageManifest.contains("name: \"QuillShimsTests\",\n                dependencies: testDeps,\n                swiftSettings: appSwiftSettings"))
        #expect(fetchUpstream.contains("QuillURLSessionFixtures.upload(for: request, from: request.httpBody!, fallbackSession: urlSession, delegate:"))
        #expect(quillKit.contains("public static func upload("))
        #expect(quillKit.contains("var requestBodyJSON: Data?"))
        #expect(quillKit.contains("requestBody(request.httpBody, matchesJSON: requestBodyJSON)"))
        #expect(mastodonFixtures.contains("\"path\": \"/api/v2/media\""))
        #expect(mastodonFixtures.contains("\"path\": \"/api/v1/media/quill-composer-upload-1\""))
        #expect(mastodonFixtures.contains("\"requestBody\": {\n        \"description\": \"quillui linux media description\""))
        #expect(onChangeModifier.contains("public struct InitialOnChangeView"))
        #expect(onChangeModifier.contains("public struct InitialOnChangeTwoArgView"))
        #expect(!designSystemCompat.contains("initial: Bool"))
        #expect(asyncImage.contains("@ObservedObject private var loader: AsyncImageLoader"))
        #expect(asyncImage.contains("AsyncImageLoaderRegistry.shared.loader(for: url)"))
        #expect(asyncImage.contains("contentForPhase(loader.phaseStartingIfNeeded())"))
        #expect(environment.contains("private enum PresentationDismissTaskLocal"))
        #expect(environment.contains("public func swiftOpenUIResolvePresentationDismissAction"))
        ...
        #expect(qtRenderer.contains("swiftOpenUIResolvePresentationDismissAction"))
        #expect(qtRenderer.contains("await withTaskEnvironment(captured.environment)"))
        #expect(winRenderer.contains("swiftOpenUIResolvePresentationDismissAction"))
        #expect(fileImporter.contains("initial: true"))
    ...
    func deferredCallbackTaskEnvironmentsRemainReproducible() throws {
        ...
        #expect(renderer.contains("guard !hasCurrentTask else"))
        ...
        #expect(renderTests.contains("GTKNativeCallbackProbe"))
        #expect(renderTests.contains("Thread.detachNewThread"))
    ...
    func vendoredGTKScrollViewReaderUsesDeferredIDAdjustmentScrolling() throws {
        ...
        #expect(viewHost.contains("gtkWithForcedStateIdentityNamespace(stateIdentityNamespace)"))
        #expect(viewHost.contains("gtkWithOwnedDescriptorLifecyclePayloads"))
