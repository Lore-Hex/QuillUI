...
struct SourceHygieneTests {
    ...
    func qtManifestAvoidsPkgConfigProhibitedFlagWarnings() throws {
        ...
        #expect(manifest.contains("let quillCanonicalLinuxAppProducts: [Product] = ["))
        ...
        #expect(manifest.contains("let quillGenericQtAppTargets: [(name: String, path: String)] = ["))
        #expect(manifest.contains("] + quillGenericQtAppTargets.map { spec in"))
        #expect(manifest.contains("name: spec.name"))
        #expect(manifest.contains("path: spec.path"))
