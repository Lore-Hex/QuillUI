...
struct SourceHygieneTests {
    ...
    func linuxInteractionSmokeTargetsShareOneViewSurface() throws {
        ...
        #expect(enchantedQtHost.contains("QWidget *markdownMessageWidget(const QString &markdown)"))
        #expect(enchantedQtHost.contains("layout->setSpacing(9)"))
        #expect(enchantedQtHost.contains("addMarkdownBlocks(layout, markdown)"))
        #expect(enchantedQtHost.contains("layout->addWidget(markdownMessageWidget(stringValue(message, \"content\")))"))
