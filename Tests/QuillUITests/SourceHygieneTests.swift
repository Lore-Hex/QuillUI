...
struct SourceHygieneTests {
    ...
    func linuxInteractionSmokeTargetsShareOneViewSurface() throws {
        ...
        #expect(backendScript.contains("print_app_log_on_verifier_failure()"))
        #expect(backendScript.contains("tail -n \"${QUILLUI_BACKEND_INTERACTION_APP_LOG_LINES:-80}\" \"$APP_LOG_PATH\""))
        #expect(backendScript.contains(">\"$APP_LOG_PATH\" 2>&1 &"))
        #expect(!backendScript.contains("/tmp/quillui-gtk-interaction-app.log"))
        #expect(backendScript.contains("quill-wireguard-qt) INTERACTION_MODE=\"tunnel-name-edit\""))
        #expect(backendScript.contains("wireguard_qt_import_configuration()"))
        ...
        #expect(backendScript.contains("WireGuard Qt import fixture is missing"))
        #expect(backendScript.contains("tunnel-name-edit|name-edit"))
        #expect(backendScript.contains("import-paste|paste-import"))
        #expect(backendScript.contains("import-file|file-import"))
        #expect(backendScript.contains("wireguard_qt_import_configuration_file()"))
        #expect(backendScript.contains("QUILLUI_WIREGUARD_QT_IMPORT_CONFIGURATION_FILE_ON_START"))
        #expect(backendScript.contains("QUILLUI_BACKEND_NAME_CLICK_X"))
        #expect(backendScript.contains("QUILLUI_BACKEND_IMPORT_EDITOR_X"))
        #expect(backendScript.contains("import_configuration=\"$(wireguard_qt_import_configuration)\" || exit $?"))
