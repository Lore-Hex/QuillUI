...
struct LinuxBackendAppMatrixTests {
    ...
    func coversEachUserFacingAppProductOnce() throws {
        ...
        #expect(backendProductBuildScript.contains("quillui_build_backend_product()"))
        ...
        #expect(backendProductBuildScript.contains("if [[ \"$build_backend\" == \"qt\" ]] && grep -Eq \"warning:|prohibited flag|Invalid Exclude\" <<<\"$output\"; then"))
        #expect(backendProductBuildScript.contains("native Qt products must stay warning-clean"))
        #expect(backendProductBuildScript.contains("quillui_build_backend_product \"$product\" \"$build_backend\""))
