...
struct CoreContractMatrixTests {
    ...
    func enchantedQtNativeTargetContracts() throws {
        ...
        #expect(nativeShim.contains("QLineEdit, QComboBox, QPlainTextEdit { background: %1; color: %2; border: 1px solid #CDD5CA; border-radius: %3; padding: %4; }"))
        ...
        #expect(nativeShim.contains("border-radius: %10;"))
        #expect(!nativeShim.contains("min-width: 9px; max-width: 9px; min-height: 9px; max-height: 9px; border-radius: 4px;"))
        #expect(nativeShim.contains(".arg(selected, ink, card, success, warning, dropTarget, canvas, primary, statusDotSize, statusDotRadius)"))
