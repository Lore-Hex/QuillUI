...
struct CoreContractMatrixTests {
    ...
    func emptyConversationPromptGridMirrorsMacLayout() throws {
        ...
        let quillPaintButtonStyle = try packageSource("Sources/QuillUI/QuillPaintButtonStyle.swift")
        ...
        expectContains(rootView, ".quillPaint(.macDefault)")
        expectDoesNotContain(rootView, ".background(model.isLoading ? QuillColors.warning : QuillColors.primary)")
        ...
        expectContains(controls, ".quillPaint(.macBordered)")
        expectContains(quillPaintButtonStyle, "public enum QuillPaintButtonChrome")
        expectContains(quillPaintButtonStyle, "case macDefault")
        expectContains(quillPaintButtonStyle, "case macBordered")
        expectContains(quillPaintButtonStyle, "buttonStyle(.borderedProminent)")
        expectContains(quillPaintButtonStyle, "buttonStyle(.bordered)")
        expectContains(quillPaintButtonStyle, "buttonStyle(ButtonStyleType.quillPaintMacDefault)")
        expectContains(quillPaintButtonStyle, "buttonStyle(ButtonStyleType.quillPaintMacBordered)")
        expectContains(gtkPatchScript, "views.count <= 64")
        expectContains(gtkPatchScript, "gint(index / columns)")
        expectContains(gtkPatchScript, "case quillPaintMacDefault")
    ...
    func enchantedQtNativeTargetContracts() throws {
        ...
        expectContains(coreApp, "Double(EnchantedVisualMetrics.defaultWindowWidth)")
        expectContains(coreApp, "Double(EnchantedVisualMetrics.defaultWindowHeight)")
        ...
        expectContains(nativeShim, "layout->setContentsMargins(\n        emptyStatePadding,\n        emptyStatePadding,\n        emptyStatePadding,\n        emptyStatePadding\n    )")
        expectContains(macOSRootView, "VStack(alignment: .center, spacing: CGFloat(EnchantedVisualMetrics.emptyStateSpacing))")
        expectContains(nativeShim, "layout->setSpacing(styleInt(style, \"emptyStateSpacing\"));\n    layout->setAlignment(Qt::AlignTop | Qt::AlignLeft)")
        expectContains(macOSRootView, "VStack(alignment: .center, spacing: CGFloat(EnchantedVisualMetrics.emptyStateHeaderSpacing))")
        ...
        expectContains(nativeShim, "bool markdownReferenceLinkReplacement(\n    const QString &text,\n    const int index,")
        expectContains(nativeShim, "QString replaceMarkdownReferenceLinks(const QString &text)")
        ...
        expectContains(nativeShim, "text = replaceMarkdownReferenceImages(text);\n    text = replaceMarkdownReferenceLinks(text)")
        expectContains(nativeShim, "text = replaceMarkdownReferenceLinks(text);\n    text = replaceMarkdownAutolinks(text)")
        ...
        expectContains(nativeShim, "QLabel#conversationTitle { color: %1; font-size: %2; font-weight: %3; }")
        expectContains(nativeShim, "QFrame#conversationSelectionDot { background: %4;")
        expectDoesNotContain(nativeShim, "QLabel#conversationPreview")
        ...
        expectDoesNotContain(macOSRootView, "Text(conversation.lastMessage)")
        expectDoesNotContain(macOSRootView, ".font(.system(size: CGFloat(EnchantedTypography.conversationPreviewFontSize)))")
        expectContains(macOSRootView, ".accessibilityLabel(conversation.title)")
        expectDoesNotContain(macOSRootView, ".accessibilityValue(conversation.lastMessage)")
        ...
        expectContains(controlsSource, "private var rowCornerRadius: CGFloat { CGFloat(MacMetrics.ListRow.cornerRadius) }")
        ...
        expectContains(controlsSource, "@State private var hoveredItemID: String?")
        expectContains(controlsSource, "let isHovered = hoveredItemID == item.id")
        expectContains(controlsSource, "let rowState = PaintControlState(isHovered: isHovered, isSelected: isSelected)")
        expectContains(controlsSource, "private var rowBackgroundColor: Color { Color(quillPaint: MacColors.controlBackground) }")
        expectContains(controlsSource, "private func rowBackgroundColor(for state: PaintControlState) -> Color")
        expectContains(controlsSource, "MacListRowPaint.effectiveFillColor(for: state)")
        expectContains(controlsSource, "MacListRowPaint.primaryTextColor(for: state)")
        expectContains(controlsSource, "MacListRowPaint.secondaryTextColor(for: state)")
        expectContains(controlsSource, ".foregroundColor(rowTitleColor(for: rowState))")
        expectContains(controlsSource, ".foregroundColor(rowPreviewColor(for: rowState))")
        expectContains(controlsSource, ".background(rowBackgroundColor(for: rowState))")
        expectContains(controlsSource, ".onHover { hovering in")
        expectContains(controlsSource, "hoveredItemID = hovering ? item.id : nil")
        ...
        expectContains(macOSRootView, ".quillPaint(.macDefault)")
...
private let inlineCases: [TextCase] = [
    ...
    TextCase(input: "Use ``code`` and ```swift```", expected: "Use code and swift"),
    TextCase(input: "Use `` `literal` `` span", expected: "Use `literal` span"),
    TextCase(input: "Try `**literal**` and `&amp;`", expected: "Try **literal** and &amp;"),
    TextCase(input: "Keep unmatched `` marker", expected: "Keep unmatched `` marker"),
    TextCase(input: "Keep a literal ** marker", expected: "Keep a literal ** marker"),
    TextCase(input: "Keep a literal ` marker", expected: "Keep a literal ` marker"),
    TextCase(input: "Keep a literal ~~ marker", expected: "Keep a literal ~~ marker"),
...
private let blockCases: [BlockCase] = [
    ...
    BlockCase(markdown: "Line one  \nLine two", kind: .paragraph, text: "Line one\nLine two"),
    BlockCase(markdown: "Slash break\\\nnext", kind: .paragraph, text: "Slash break\nnext"),
