...
struct AppKitLoweringTests {
    ...
    func nonisolatedLeavesOthersAlone() {
        ...
        // designated init this pass never annotates. A separate synthesized no-arg
        // init may still be needed for the NSObject-direct class itself.
    ...
    @Test("forwardingTarget(for:) override is preserved for UIResponder class-body member")
    ...
    @Test("forwardingTarget override with a class-body super call is preserved")
    func forwardingTargetPreservedWithSuperCall() {
        ...
        // real UIResponder member, so the override is valid and should stay intact.
        let source = """
    ...
    func initWithMainActorBodyLeftAlone() {
        ...
        // No second init is synthesized because the explicit no-arg override already
        // exists; keeping it actor-isolated is the point of this case.
        #expect(!lowered.contains("@available(*, unavailable)"))
    ...
    func subclassSynthesizesNonisolatedInitToMatchBase() {
        ...
        // Recent annotated.
        #expect(lowered.contains("nonisolated override public init()"))
        // Installed gets a synthesized unavailable nonisolated override init() despite
        // having init(arg:), because it has initialized stored properties.
