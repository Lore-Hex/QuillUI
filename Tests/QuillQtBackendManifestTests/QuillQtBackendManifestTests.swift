...
struct QuillQtBackendManifestTests {
    ...
    func qtBackendRegistersRealQtModeTestTarget() throws {
        ...
        #expect(manifest.contains("let quillFoundationDependencies: [Target.Dependency] = [\"QuillKit\", \"CGdkPixbuf\"]"))
        #expect(manifest.contains("let quillFoundationSwiftSettings: [SwiftSetting] = [\n    .unsafeFlags(gdkPixbufSwiftImporterFlags)\n]"))
    ...
    func genericQtAppCatalogSnapshotsMatchProductRoster() throws {
        ...
        for product in Self.expectedGenericQtCatalogProducts {
            ...
            #expect(decodedSnapshot.presentation == expectation.snapshot.presentation)
    ...
    @Test("Generated Enchanted Qt catalog uses the shared chat presentation")
    func generatedEnchantedQtCatalogUsesSharedChatPresentation() throws {
        let snapshot = QuillGenericQtAppCatalog.enchantedUpstreamSlice
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        ...
        for snapshot in [snapshot, decodedSnapshot] {
            assertEnchantedSliceUsesSharedMetrics(snapshot)
            #expect(snapshot.presentation == .chat)
            #expect(snapshot.windowTitle == "Quill Chat")
            #expect(snapshot.sidebarTitle == "Quill Chat")
            #expect(snapshot.emptyStateTitle == "Quill")
            #expect(snapshot.emptyStateSubtitle == EnchantedCopy.emptyStateSubtitle)
            #expect(snapshot.prompts.map(\.title) == EnchantedPromptCatalog.visibleEmptyConversationPrompts.map(\.title))
            #expect(snapshot.prompts.map(\.systemImage) == EnchantedPromptCatalog.visibleEmptyConversationPrompts.map(\.systemImage))
            #expect(snapshot.bottomNavigation.map(\.title) == [
                EnchantedCopy.completionsTitle,
                EnchantedCopy.shortcutsTitle,
                EnchantedCopy.settingsTitle
            ])
            #expect(snapshot.bottomNavigation.map(\.systemImage) == [
                EnchantedIcon.completions,
                EnchantedIcon.shortcuts,
                EnchantedIcon.settings
            ])
            #expect(snapshot.composerPlaceholder == EnchantedCopy.composerPlaceholder)
    ...
    func genericQtSnapshotsDecodeLegacyPayloadDefaults() throws {
        ...
        #expect(snapshot.presentation == .standard)
        #expect(snapshot.emptyStateTitle.isEmpty)
        #expect(snapshot.emptyStateSubtitle.isEmpty)
        #expect(snapshot.prompts.isEmpty)
        #expect(snapshot.bottomNavigation.isEmpty)
        #expect(snapshot.composerPlaceholder.isEmpty)
        #expect(snapshot.noticeTitle.isEmpty)
        #expect(snapshot.noticeBody.isEmpty)
        #expect(snapshot.noticeActionTitle.isEmpty)
        ...
        #expect(snapshot.style.headerColor == QuillGenericQtAppSnapshot.Style.desktop.headerColor)
        #expect(snapshot.style.promptCardColor == QuillGenericQtAppSnapshot.Style.desktop.promptCardColor)
        #expect(snapshot.style.noticeColor == QuillGenericQtAppSnapshot.Style.desktop.noticeColor)
        ...
        #expect(snapshot.style.emptyStateWordmarkFontSize == QuillGenericQtAppSnapshot.Style.desktop.emptyStateWordmarkFontSize)
        #expect(snapshot.style.headerHeight == QuillGenericQtAppSnapshot.Style.desktop.headerHeight)
        #expect(snapshot.style.promptCardWidth == QuillGenericQtAppSnapshot.Style.desktop.promptCardWidth)
        #expect(snapshot.style.composerEditorRadius == QuillGenericQtAppSnapshot.Style.desktop.composerEditorRadius)
    ...
    func genericQtStyleDecodingDefaultsMissingFields() throws {
        ...
        #expect(style.headerColor == QuillGenericQtAppSnapshot.Style.desktop.headerColor)
        #expect(style.promptCardColor == QuillGenericQtAppSnapshot.Style.desktop.promptCardColor)
        #expect(style.noticeColor == QuillGenericQtAppSnapshot.Style.desktop.noticeColor)
        ...
        #expect(style.emptyStateWordmarkFontWeight == QuillGenericQtAppSnapshot.Style.desktop.emptyStateWordmarkFontWeight)
        #expect(style.headerPadding == QuillGenericQtAppSnapshot.Style.desktop.headerPadding)
        #expect(style.emptyStateMaxWidth == QuillGenericQtAppSnapshot.Style.desktop.emptyStateMaxWidth)
        #expect(style.promptGridColumns == QuillGenericQtAppSnapshot.Style.desktop.promptGridColumns)
        #expect(style.composerMaxWidth == QuillGenericQtAppSnapshot.Style.desktop.composerMaxWidth)
    ...
    private func assertEnchantedSliceUsesSharedMetrics(_ snapshot: QuillGenericQtAppSnapshot) {
        ...
        #expect(snapshot.style.headerColor == EnchantedPalette.headerColor)
        #expect(snapshot.style.promptCardColor == EnchantedPalette.cardQuietColor)
        #expect(snapshot.style.noticeColor == EnchantedPalette.noticeColor)
        ...
        #expect(snapshot.style.emptyStateWordmarkFontSize == EnchantedTypography.emptyStateWordmarkFontSize)
        #expect(snapshot.style.emptyStateWordmarkFontWeight == EnchantedTypography.emptyStateWordmarkFontWeight)
        #expect(snapshot.style.headerHeight == EnchantedVisualMetrics.headerHeight)
        #expect(snapshot.style.headerPadding == EnchantedVisualMetrics.headerPadding)
        #expect(snapshot.style.headerSpacing == EnchantedVisualMetrics.headerSpacing)
        #expect(snapshot.style.headerTitleSpacing == EnchantedVisualMetrics.headerTitleSpacing)
        #expect(snapshot.style.contentPadding == EnchantedVisualMetrics.contentPadding)
        #expect(snapshot.style.emptyStateMaxWidth == EnchantedVisualMetrics.emptyStateMaxWidth)
        #expect(snapshot.style.emptyStatePadding == EnchantedVisualMetrics.emptyStatePadding)
        #expect(snapshot.style.emptyStateSpacing == EnchantedVisualMetrics.emptyStateSpacing)
        #expect(snapshot.style.emptyStateHeaderSpacing == EnchantedVisualMetrics.emptyStateHeaderSpacing)
        #expect(snapshot.style.promptGridColumns == EnchantedVisualMetrics.promptGridColumns)
        #expect(snapshot.style.promptGridSpacing == EnchantedVisualMetrics.promptGridSpacing)
        #expect(snapshot.style.promptCardWidth == EnchantedVisualMetrics.promptCardWidth)
        #expect(snapshot.style.promptCardHeight == EnchantedVisualMetrics.promptCardHeight)
        #expect(snapshot.style.promptGridWidth == EnchantedVisualMetrics.promptGridWidth)
        #expect(snapshot.style.promptButtonPadding == EnchantedVisualMetrics.promptButtonPadding)
        #expect(snapshot.style.promptButtonRadius == EnchantedVisualMetrics.promptButtonRadius)
        ...
        #expect(snapshot.style.composerMinWidth == EnchantedVisualMetrics.composerMinWidth)
        #expect(snapshot.style.composerMaxWidth == EnchantedVisualMetrics.composerMaxWidth)
        #expect(snapshot.style.composerPadding == EnchantedVisualMetrics.composerPadding)
        #expect(snapshot.style.composerSpacing == EnchantedVisualMetrics.composerSpacing)
        #expect(snapshot.style.promptRowSpacing == EnchantedVisualMetrics.promptRowSpacing)
        #expect(snapshot.style.composerMinHeight == EnchantedVisualMetrics.composerMinHeight)
        #expect(snapshot.style.composerMaxHeight == EnchantedVisualMetrics.composerMaxHeight)
        #expect(snapshot.style.composerEditorRadius == EnchantedVisualMetrics.composerEditorRadius)
        #expect(snapshot.style.composerSendButtonMinWidth == EnchantedVisualMetrics.composerSendButtonMinWidth)
