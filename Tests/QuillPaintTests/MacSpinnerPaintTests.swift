...
struct MacSpinnerPaintTests {
    ...
    func normalState() {
        ...
        if case let .strokeLine(start, end, color, lineWidth, lineCap) = ctx.calls[0] {
            ...
            #expect(color == MacColors.spinnerSpoke.withAlpha(1.0))
        ...
        if case let .strokeLine(_, _, color, _, _) = ctx.calls[1] {
            #expect(abs(color.alpha - 0.85) < 0.0001)
        }
        
        // Spoke 6 is at 6 o'clock
        if case let .strokeLine(_, _, color, _, _) = ctx.calls[6] {
    ...
    func disabledState() {
        ...
        if case let .strokeLine(_, _, color, _, _) = ctx.calls[0] {
