...
struct QuillDataSourceLoweringTests {
    ...
    func visualSmokeExposesOptInMacReferenceLandmarks() throws {
        ...
        #expect(chatViewTemplate.contains("guard !json, !messages.isEmpty else { copyChat(json); return }"))
        #expect(chatViewTemplate.contains(#"Clipboard.shared.setString(messages.map { "\($0.role.capitalized): \($0.content)" }.joined(separator: "\n\n"))"#))
