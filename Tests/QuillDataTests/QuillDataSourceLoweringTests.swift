...
struct QuillDataSourceLoweringTests {
    ...
    func swiftOpenUIGTKPatchKeepsEnchantedFixesGeneric() throws {
        ...
        #expect(patchedRenderer.contains("itemCount <= 64"))
        #expect(patchedRenderer.contains("configuration.adaptiveMinimum == 0"))
        #expect(patchedRenderer.contains("renderItem: (Int) -> UnsafeMutablePointer<GtkWidget>"))
        #expect(patchedRenderer.contains("let staticRenderItem: (Int) -> UnsafeMutablePointer<GtkWidget>"))
        #expect(patchedRenderer.contains("renderItem: staticRenderItem"))
