...
struct QuillDataSourceLoweringTests {
    ...
    func visualSmokeExposesOptInMacReferenceLandmarks() throws {
        ...
        #expect(chatViewTemplate.contains("private static let copySource = QuillChatCopySource<MessageSD>()"))
        #expect(chatViewTemplate.contains("let _ = Self.copySource.update(messages)"))
        #expect(chatViewTemplate.contains("Self.copySource.copy(asJSON: json, role: \\.role, content: \\.content, fallback: copyChat)"))
