...
struct QuillDataSourceLoweringTests {
    ...
    func visualSmokeExposesOptInMacReferenceLandmarks() throws {
        ...
        #expect(chatViewTemplate.contains("@State private var copySource = QuillChatCopySource<MessageSD>()"))
        #expect(chatViewTemplate.contains("let _ = copySource.update(messages)"))
        #expect(chatViewTemplate.contains("copySource.copy(asJSON: json, role: \\.role, content: \\.content, fallback: copyChat)"))
