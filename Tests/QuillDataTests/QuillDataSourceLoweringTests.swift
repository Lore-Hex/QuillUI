...
struct QuillDataSourceLoweringTests {
    ...
    func visualSmokeExposesOptInMacReferenceLandmarks() throws {
        ...
        #expect(sidebarTemplate.contains("onCompletions: { showCompletions.toggle() }"))
        #expect(sidebarTemplate.contains("onShortcuts: { showKeyboardShortcutas.toggle() }"))
        #expect(sidebarTemplate.contains("onSettings: onSettingsTap"))
