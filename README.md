...
- **Network parity pocket:** [Network function rows](docs/apple-package-function-coverage.md#network) list the narrow API rows currently at `Parity`; path monitor initial state and pre-start cancellation, scoped service endpoint descriptions, and associated interface values are now pinned there, while broader transport, DNS, TLS, connection, listener, VPN, and live monitoring work is still incomplete.
...
| `Network` and `NetworkExtension` | Partial overall; selected `Network` functions at Parity | Imports and app-facing types compile for ports such as WireGuard. `IPv4Address` and `IPv6Address` parsing, constants, data initializer lengths, classifier boundary behavior, multicast scope, IPv4 mapping, scoped interface literals, string/debug text, address equality/hash behavior, and absence of Linux-only public address classifiers are now covered by Apple-observed parity tests. `NWPathMonitor` initial `currentPath` and pre-start `cancel()` state, `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWInterface` values returned by scoped address/host parsing, `NWEndpoint.Port` parsing/known constants/debug text/equality/hash behavior with seeded fuzz coverage, and the `NWEndpoint.Host.init(_:)` classification/description/equality/hash contract for current scoped and unscoped edge cases now match Apple-observed behavior, including scoped parsed-to-direct equality and scoped-vs-unscoped inequality. `NWEndpoint.hostPort`, `.service`, and `.unix` description/debug/equality/hash behavior now covers Apple-observed DNS-SD service-name escaping, `_tcp`/`_udp` type formatting, empty-name/domain cases, invalid-type concatenation, leading/internal domain dot behavior, exact endpoint equality, scoped host-port equality, and hash coherence. | Real VPN control, tunnel lifecycle, system extension behavior, DNS/TLS/TCP/UDP behavior, connections, listeners, live path monitoring, synthetic constructed interfaces, and live interface/path probing are not implemented. |
...
| Web, network, and extensions | `QuillWebKit`, `Network`, `NetworkExtension` | Web/network imports and selected types compile. Network address parsing, address constants/properties, address equality/hash behavior, scoped and unscoped endpoint host address/name parsing/debug/equality/hash behavior, port construction with seeded fuzz coverage plus equality/hash semantics, well-known port constants, path monitor initial and pre-start cancel state, path status/reason/type string/equality/hash semantics, and `NWEndpoint` host-port/service/Unix path descriptions/debug/equality/hash behavior are parity-tested for current Apple-observed value behavior; real web rendering, VPN tunnels, system extensions, DNS/TLS/TCP/UDP behavior, connections, listeners, and network monitoring are not implemented. |
