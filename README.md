...
| Diagnostics and unsupported modifiers | Usable fallback | Unsupported fallbacks such as `symbolEffect`, `matchedGeometryEffect`, `mask`, `contentShape`, `Image.renderingMode`, and `Form.formStyle` record `QuillCompatibilityDiagnostics`; keyboard/text-entry modifiers plus generic `View.imageScale`, `minimumScaleFactor`, `listRowInsets`, and `listRowSeparator` now preserve metadata for backend rendering/layout work. | These APIs may compile without changing visuals or behavior until a backend implementation is added. |
