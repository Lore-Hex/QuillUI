...
| Diagnostics and unsupported modifiers | Usable fallback | Unsupported fallbacks such as `symbolEffect`, `matchedGeometryEffect`, `mask`, `contentShape`, generic `View.imageScale`, `minimumScaleFactor`, keyboard/text-entry modifiers, `Image.renderingMode`, and `Form.formStyle` record `QuillCompatibilityDiagnostics`; generic `View.imageScale` and `minimumScaleFactor` now preserve metadata for backend rendering/layout work. | These APIs may compile without changing visuals or behavior until a backend implementation is added. |
