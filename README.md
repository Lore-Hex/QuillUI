...
| Diagnostics and unsupported modifiers | Usable fallback | Unsupported fallbacks such as `symbolEffect`, `matchedGeometryEffect`, `mask`, `contentShape`, `minimumScaleFactor`, keyboard/text-entry modifiers, `Image.renderingMode`, and `Form.formStyle` record `QuillCompatibilityDiagnostics`; `minimumScaleFactor` now preserves its factor as view metadata for backend layout work. | These APIs may compile without changing visuals or behavior until a backend implementation is added. |
