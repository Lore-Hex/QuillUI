...
**Function-by-function coverage:** start with the [Apple package function coverage ledger](docs/apple-package-function-coverage.md)
for the function-by-function complete/incomplete status of each cloned Apple
package and app-facing package clone.
...
- [Apple package function coverage](docs/apple-package-function-coverage.md): the primary function-by-function complete/incomplete ledger for SwiftUI, SwiftData, AppKit/UIKit, Network, media/service kits, system kits, and app-facing package clones.
...
| Web, network, media, and service Apple kits | Selected compile or fallback shapes for `WebKit`, `AuthenticationServices`, `Network`, `NetworkExtension`, `AVFoundation`, `Speech`, `PhotosUI`, `Charts`, `StoreKit`, and `TipKit`; `Network` address parsing, address constants, address classifier properties, port parsing, well-known port constants, path status/reason/type, unscoped endpoint-host classification, and selected endpoint description/debug value APIs now have Apple-checked parity rows. | Real web rendering, authentication sessions, DNS/TLS/TCP/UDP behavior, VPN tunnels, media I/O, speech, photo library, chart rendering, purchases, tip persistence, and scoped IPv6 endpoint interface metadata. |
...
| `Network` and `NetworkExtension` | Partial overall; selected `Network` functions at Parity | Imports and app-facing types compile for ports such as WireGuard. `IPv4Address` and `IPv6Address` parsing, constants, classifier properties, multicast scope, IPv4 mapping, debug text, plus `NWPath.Status`, `NWPath.UnsatisfiedReason`, `NWInterface.InterfaceType`, `NWEndpoint.Port` parsing/known constants/debug text, and the unscoped `NWEndpoint.Host.init(_:)` classification/description contract now match Apple-observed behavior for tested edge cases. Common `NWEndpoint` value descriptions/debug text are covered by the same shared macOS/Linux contract. | Real VPN control, tunnel lifecycle, system extension behavior, DNS/TLS/TCP/UDP behavior, connections, listeners, network monitoring, and scoped IPv6 endpoint interface metadata are not implemented. |
...
| Web, network, and extensions | `QuillWebKit`, `Network`, `NetworkExtension` | Web/network imports and selected types compile. Network address parsing, address constants/properties, unscoped endpoint host address/name parsing/debug text, port construction, well-known port constants, path status/reason/type descriptions, and selected endpoint descriptions/debug text are parity-tested for current Apple-observed value behavior; real web rendering, VPN tunnels, system extensions, DNS/TLS/TCP/UDP behavior, scoped IPv6 endpoint interface metadata, connections, listeners, and network monitoring are not implemented. |
