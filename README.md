...
| Web, network, and extensions | `QuillWebKit`, `Network`, `NetworkExtension` | Web/network imports and selected types compile. Network address parsing, address constants/properties, scoped and unscoped endpoint host address/name parsing/debug text, port construction with seeded fuzz coverage, well-known port constants, path status/reason/type descriptions, and `NWEndpoint` host-port/service/Unix path descriptions/debug text are parity-tested for current Apple-observed value behavior; real web rendering, VPN tunnels, system extensions, DNS/TLS/TCP/UDP behavior, connections, listeners, and network monitoring are not implemented. |
