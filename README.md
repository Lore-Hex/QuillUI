...
- **Porting lessons (read first):** [docs/porting-lessons.md](docs/porting-lessons.md) — hard-won, transferable lessons for bringing real Apple/upstream code to Linux: the core principle (**clone the missing macOS API into the lib, not the app**), how "full port" works (faithful reimpl UI + verbatim-vendored frameworks), the vendoring pattern, a running catalog of macOS-vs-Linux gaps that only Linux CI catches, CI topology, the `swift test` harness quirks, and merge/branch discipline.
