...
- [Network](docs/apple-package-function-coverage.md#network): the current first
  ...
  `NWParameters` factory/initializer text plus policy enum/default/setter/debug
  behavior, and
  Apple-checked protocol-surface conformance rows.
...
- [Network function coverage](docs/apple-package-function-coverage.md#network): current
  ...
  localized error formatting; `NWParameters` policy enums, defaults, setter
  normalization, local endpoint debug formatting, and traffic, multipath,
  proxy, and DNSSEC debug segments are pinned by shared Apple/Linux tests.
