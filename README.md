...
| Symbols, images, and rendering | Partial | SF-symbol style names map through `quillSystemImageName`, `Image(data:)` deduplicates payloads, PNG/TIFF image round trips are tested through gdk-pixbuf, and `ImageRenderer` can render supported color payloads. | General SwiftUI view rendering to images, full symbol fidelity, animated images, and every platform image format are not complete. |
...
| Diagnostics and unsupported modifiers | Usable fallback | Unsupported fallbacks such as `symbolEffect`, `matchedGeometryEffect`, `mask`, `contentShape`, keyboard/text-entry modifiers, `Image.renderingMode`, and `Form.formStyle` record `QuillCompatibilityDiagnostics`. | These APIs may compile without changing visuals or behavior until a backend implementation is added. |
