...
# (QuillCodeApp ~1580 modules, GeneratedSwiftUILinuxApp ~1800) cold-build right
# at ~10 minutes and were being SIGTERM'd at ~99% compiled (exit 124) on loaded
# CI runners, intermittently reddening main. Give the cold build ample headroom:
# this only affects the failure/hang path (a genuinely hung smoke is still
# bounded, and a crash exits fast with a signal code, never 124), so raising it
# costs nothing on passing runs while removing the flake.
