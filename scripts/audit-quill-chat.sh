...
- Interpretation: the prototype slice is scaffolding, not the desired port shape. The real target is the full-source generated path plus a tiny Linux entry point, then applying the same approach to this local $swift_files-file Quill Chat tree with an app-side shim budget of <= 100 lines.
