...
- Generated Linux app build: \`scripts/build-swiftui-linux-app.sh --profile enchanted-full-source --source-dir "$APP_DIR" --app-type EnchantedApp --product-name quill-chat-linux\` is the generic build entry point; generated app launchers can use the backend-neutral QuillUI entry or the QuillUIGtk/QuillUIQt facades; profiles are discovered from \`scripts/profiles/*.sh\`; \`scripts/build-quill-chat-linux.sh\` is only a convenience wrapper for this app.
