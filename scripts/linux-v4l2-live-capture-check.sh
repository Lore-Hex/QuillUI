...
# ffmpeg, and runs the device-gated `liveCaptureDeliversConvertedBGRAFrames`
# test (Tests/QuillCompatibilityModuleTests/V4L2ConversionTests.swift) against
# it. That test drives QuillV4L2Camera end-to-end: QUERYCAP discovery ->
# S_FMT/G_FMT YUYV -> mmap ring -> STREAMON -> DQBUF loop -> YUYV->BGRA ->
# CVPixelBuffer, proving SolderScope's camera works against a live device.
#
# SKIPS cleanly (exit 0) wherever a real V4L2 device can't be created — the
# standard Linux CI runs inside a container that cannot load kernel modules,
# and OrbStack/LinuxKit kernels lack v4l2loopback. It is meaningful on a
# real-kernel host or a non-containerized runner that has v4l2loopback-dkms +
# linux-headers + ffmpeg installed (see the README/dev-VM notes). Run it from a
# real-kernel Linux dev VM:  scripts/linux-v4l2-live-capture-check.sh
set -uo pipefail
...
skip() { echo "SKIP linux-v4l2-live-capture: $1"; exit 0; }
...
# node as a capture device to the reader (the test).
...
echo "=== virtual webcam up on /dev/video0; running live-capture test ==="
QUILLUI_LINUX_BACKEND="${QUILLUI_LINUX_BACKEND:-gtk}" \
  "$ROOT_DIR/scripts/linux-swift-test.sh" --scratch-path "$SCRATCH_PATH" \
  --filter liveCaptureDeliversConvertedBGRAFrames
