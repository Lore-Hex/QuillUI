...
//
//  Quill bring-up: vendored verbatim except the `wrapped(_:_:)` and
//  `account(from:)` convenience helpers, which reference the `Account` class /
//  `AccountManager` (not vendored yet). They are restored verbatim once the
//  Account model lands; the error type and its messages are complete.
//
//  `import QuillFoundation` pulls in the Linux Foundation-clone gap-fills
//  (e.g. NSString.localizedStringWithFormat, absent from swift-corelibs-
//  foundation) so the body below stays verbatim upstream on every platform.
//
