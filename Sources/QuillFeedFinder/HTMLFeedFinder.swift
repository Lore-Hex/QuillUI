...
//  FeedFinder module. `import RSParser` is retargeted to `QuillRSParser` (the
//  vendored RSParser shim) and `QuillRSCoreShim` is added because that shim does
//  not `@_exported import` RSCore the way upstream RSParser does (so the
//  `String.normalizedURL` helper resolves). Body unchanged. This is the live,
//  network-free part of FeedFinder — it detects feeds inside already-fetched
//  HTML; `FeedFinder.find()` (which needs RSWeb's Downloader) is deferred.
//
//  Refresh: re-copy from
//  .upstream/netnewswire/Modules/FeedFinder/Sources/FeedFinder/HTMLFeedFinder.swift
//
