...
//  live RSWeb clone (Sources/QuillRSWebShim, the `RSWeb` module). URL helpers
//  (http/https classification, query appending, browser-prep). Foundation-only;
//  uses `localeForLowercasing` (partial-vendored from SpecialCases.swift). Note:
//  upstream's `absoluteStringWithHTTPOrHTTPSPrefixRemoved()` returns the scheme
//  prefix rather than the remainder (an upstream bug) — vendored as-is.
//
