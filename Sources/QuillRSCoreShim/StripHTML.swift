...
//  (stdlib only — String UTF-8 storage), so it compiles unchanged on macOS and
//  Linux. This is RSCore's authentic `String.strippingHTML`, distinct from
//  QuillFoundation's app-facing `HTMLText`; both coexist — the RSCore one exists
//  so real RSCore source compiles against the clone.
//
